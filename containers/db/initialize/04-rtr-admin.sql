...
BEGIN
    ...
    BEGIN
        ALTER SERVER ROLE [sysadmin] ADD MEMBER [$(RTR_ADMIN_NAME)];
        PRINT 'Granted [sysadmin] to [$(RTR_ADMIN_NAME)] (Local Instance)';
    END
    ELSE
    BEGIN
        -- On RDS, grant restricted server roles
        ALTER SERVER ROLE [setupadmin] ADD MEMBER [$(RTR_ADMIN_NAME)];
        ALTER SERVER ROLE [processadmin] ADD MEMBER [$(RTR_ADMIN_NAME)];
        GRANT ALTER ANY LOGIN TO [$(RTR_ADMIN_NAME)];
        GRANT CREATE ANY DATABASE TO [$(RTR_ADMIN_NAME)];
        GRANT VIEW SERVER STATE TO [$(RTR_ADMIN_NAME)];
        PRINT 'Granted restricted deployment privileges to [$(RTR_ADMIN_NAME)] (RDS)';
    END
END
...
BEGIN
    EXEC('
        ...
        ALTER ROLE [SQLAgentOperatorRole] ADD MEMBER [$(RTR_ADMIN_NAME)];
        GRANT SELECT ON msdb.dbo.sysjobs TO [$(RTR_ADMIN_NAME)];
        
        -- RDS specific grants (ignored if procedure doesn''t exist)
        IF EXISTS (SELECT 1 FROM sys.objects WHERE name = ''rds_cdc_enable_db'')
        BEGIN
            GRANT EXECUTE ON msdb.dbo.rds_cdc_enable_db TO [$(RTR_ADMIN_NAME)];
            GRANT EXECUTE ON msdb.dbo.rds_cdc_disable_db TO [$(RTR_ADMIN_NAME)];
        END
    ');
END
GO

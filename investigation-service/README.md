...
```shell
docker ps -a -f "name=kafka$" -f "name=nbs-mssql$"
```
3. In the root of this repository execute the following command so the service uses `application-local.yaml`.
```shell
./gradlew :investigation-service:bootRun --args='--spring.profiles.active=local'
```
4. (Optional) Run the service in debug mode.
```shell
./gradlew :investigation-service:bootRun --args='--spring.profiles.active=local' --debug-jvm
```
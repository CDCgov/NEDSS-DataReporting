...
2. Activate the repository virtual environment (if available):
```bash
...
```bash
python testing-tools/local-db-tracing/generate_query_expected.py --paired-run-dir output/20260423-103745-NBS_ODSE-to-RDB_MODERN
```

Generate from a specific manifest path:

```bash
python testing-tools/local-db-tracing/generate_query_expected.py --combined-manifest output/20260423-103745-NBS_ODSE-to-RDB_MODERN/combined-manifest.json
```

Per-step cumulative verification SQL and expected rows are generated by default under `logical-<database>/step-<N>/query.sql` and `logical-<database>/step-<N>/expected.json`.
...
```bash
python testing-tools/local-db-tracing/trace_db_cdc.py [OPTIONS]
```

**Options**:
- `--server HOST`: Database server hostname
...
```bash
python testing-tools/local-db-tracing/trace_db_logical_changes.py [OPTIONS]
```
...
```bash
python testing-tools/local-db-tracing/generate_query_expected.py [OPTIONS]
```
...
2. If interrupted, resume:
   ...
   ```bash
   python testing-tools/local-db-tracing/trace_db_cdc.py
   # When prompted: "Resume from previous LSN? [Y/n]" ΓåÆ Y
   ```
...
python testing-tools/local-db-tracing/trace_db_cdc.py --batch-size 2000 --timeout 600
```
...
python testing-tools/local-db-tracing/trace_db_cdc.py --batch-size 250 --enable-cleanup

# Stream output directly (if supported)
python testing-tools/local-db-tracing/trace_db_logical_changes.py | gzip > output.jsonl.gz
```
...
python testing-tools/local-db-tracing/trace_db_cdc.py --table person --output-dir ./output/person_capture &

# Terminal 2: Capture investigation tables
python testing-tools/local-db-tracing/trace_db_cdc.py --table investigation --output-dir ./output/inv_capture &

# Terminal 3: Capture observation tables
python testing-tools/local-db-tracing/trace_db_cdc.py --table observation --output-dir ./output/obs_capture &
...
1. Increase timeout:
   ```bash
   python testing-tools/local-db-tracing/trace_db_cdc.py --timeout 900
   ```

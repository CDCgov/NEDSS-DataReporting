...
readonly PAT_UIDS='20000000,20020010,20020020,22015300'
readonly PRV_UIDS='20000010,20010010,22015400,22015410'
readonly ORG_UIDS='20000020,20030010,22015500,22015510'
...
readonly LAB_OBS_UIDS='20000120,20070010,20070011,22022000,22029401,22053011,22053501,22021011,22021021,22015600,22015601,22015602'
readonly MORB_OBS_UIDS='20000130,20080010,22015010'
...
main() {
  ...
  # Step 8.8: re-drive the interview chain post-Tier-3 so zz_interview_gap.sql's
  # nbs_answer rows (D_INTERVIEW LDF-pivot cols + D_INTERVIEW_NOTE) are processed
  # after the Tier-3 drain (run_interview_chain otherwise runs only pre-Tier-3 in
  # Steps 5/7). Idempotent.
  run_interview_chain

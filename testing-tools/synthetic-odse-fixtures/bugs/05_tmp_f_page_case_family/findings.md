...
3. `D_PATIENT.PATIENT_KEY=1` has `PATIENT_UID=NULL` (by design, the sentinel "unknown" row).
...
7. HEPATITIS_DATAMART INSERT runs against an empty source, yielding 0 rows.
...
and `sp_hepatitis_datamart_postprocessing` in that order. The patient_id
needs to point at a real `nrt_patient.patient_uid` so D_PATIENT has a
non-sentinel row.
...
4b. EXEC SP with `@debug=1`; step 3 logs `1`, downstream steps log
    `1` (identical).
5. Trace bug 5b chain (nrt_investigation.patient_id NULL, then sentinel,
   then DELETE).

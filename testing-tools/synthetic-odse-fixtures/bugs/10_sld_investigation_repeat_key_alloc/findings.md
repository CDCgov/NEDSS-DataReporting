...
**Status**: Surfaced 2026-05-21. **Fixed 2026-05-22** on
...
3. The final `INSERT INTO D_INVESTIGATION_REPEAT` reads
   ...
   `WHERE D_INVESTIGATION_REPEAT_KEY != 1` (line 1349), which is
   designed to skip the canonical sentinel row, and skips ALL of the new
   rows because they ALL have D_INVESTIGATION_REPEAT_KEY=1.
...
- `D_INVESTIGATION_REPEAT`: gains no new rows (stays at 2 sentinel
  rows + 1 baseline). Has 252 columns (8 widened by the fixture
...
- Headline coverage: 41.4% to 41.5% (+0.1pp / +7 cols populated).

...
WHERE [INVESTIGATION_KEY] = (SELECT INVESTIGATION_KEY FROM [RDB_MODERN].[dbo].INVESTIGATION WHERE INV_LOCAL_ID = N'CAS1000010004GA01')
;
...
-- Step: 2
SELECT
       dia.[ADM_IMM_NTNL_NTFBL_CNDTN]
      ,dia.[ADM_REFERRAL_BASIS_OOJ]
FROM [RDB_MODERN].[dbo].[D_INV_ADMINISTRATIVE] dia
JOIN RDB_MODERN.dbo.F_STD_PAGE_CASE f
  ON f.D_INV_ADMINISTRATIVE_KEY = dia.D_INV_ADMINISTRATIVE_KEY
JOIN RDB_MODERN.dbo.INVESTIGATION i
  ON i.INVESTIGATION_KEY = f.INVESTIGATION_KEY
WHERE i.INV_LOCAL_ID = N'CAS1000010004GA01';
;
...
SELECT
    dic.[CTT_RPT_RELATIONSHIP_TO_OP]
FROM [RDB_MODERN].[dbo].[D_INV_CONTACT] dic
JOIN RDB_MODERN.dbo.F_STD_PAGE_CASE f
  ON f.D_INV_CONTACT_KEY = dic.D_INV_CONTACT_KEY
JOIN RDB_MODERN.dbo.INVESTIGATION i
  ON i.INVESTIGATION_KEY = f.INVESTIGATION_KEY
WHERE i.INV_LOCAL_ID = N'CAS1000010004GA01';
;
...
-- Query: 7
-- Step: 2
-- This table was all _KEY values, so I'm skipping it.
SELECT 1 AS skip_this;

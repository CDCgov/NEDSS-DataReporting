-- 0: D_Patient 
SELECT
    [PATIENT_ADDED_BY],
    [PATIENT_ADDL_GENDER_INFO],
    [PATIENT_ADD_TIME],
    [PATIENT_AGE_REPORTED],
    [PATIENT_AGE_REPORTED_UNIT],
    [PATIENT_ALIAS_NICKNAME],
    [PATIENT_BIRTH_COUNTRY],
    [PATIENT_BIRTH_SEX],
    [PATIENT_CENSUS_TRACT],
    [PATIENT_CITY],
    [PATIENT_COUNTRY],
    [PATIENT_COUNTY],
    [PATIENT_COUNTY_CODE],
    [PATIENT_CURRENT_SEX],
    [PATIENT_CURR_SEX_UNK_RSN],
    [PATIENT_DECEASED_DATE],
    [PATIENT_DECEASED_INDICATOR],
    [PATIENT_DOB],
    [PATIENT_EMAIL],
    [PATIENT_ENTRY_METHOD],
    [PATIENT_ETHNICITY],
    [PATIENT_FIRST_NAME],
    [PATIENT_GENERAL_COMMENTS],
    [PATIENT_LAST_CHANGE_TIME],
    [PATIENT_LAST_NAME],
    [PATIENT_LAST_UPDATED_BY],
    [PATIENT_LOCAL_ID],
    [PATIENT_MARITAL_STATUS],
    [PATIENT_MIDDLE_NAME],
    [PATIENT_MPR_UID],
    [PATIENT_NAME_SUFFIX],
    [PATIENT_NUMBER],
    [PATIENT_NUMBER_AUTH],
    [PATIENT_PHONE_CELL],
    [PATIENT_PHONE_EXT_HOME],
    [PATIENT_PHONE_EXT_WORK],
    [PATIENT_PHONE_HOME],
    [PATIENT_PHONE_WORK],
    [PATIENT_PREFERRED_GENDER],
    [PATIENT_PRIMARY_LANGUAGE],
    [PATIENT_PRIMARY_OCCUPATION],
    [PATIENT_RACE_ALL],
    [PATIENT_RACE_AMER_IND_1],
    [PATIENT_RACE_AMER_IND_2],
    [PATIENT_RACE_AMER_IND_3],
    [PATIENT_RACE_AMER_IND_ALL],
    [PATIENT_RACE_AMER_IND_GT3_IND],
    [PATIENT_RACE_ASIAN_1],
    [PATIENT_RACE_ASIAN_2],
    [PATIENT_RACE_ASIAN_3],
    [PATIENT_RACE_ASIAN_ALL],
    [PATIENT_RACE_ASIAN_GT3_IND],
    [PATIENT_RACE_BLACK_1],
    [PATIENT_RACE_BLACK_2],
    [PATIENT_RACE_BLACK_3],
    [PATIENT_RACE_BLACK_ALL],
    [PATIENT_RACE_BLACK_GT3_IND],
    [PATIENT_RACE_CALCULATED],
    [PATIENT_RACE_CALC_DETAILS],
    [PATIENT_RACE_NAT_HI_1],
    [PATIENT_RACE_NAT_HI_2],
    [PATIENT_RACE_NAT_HI_3],
    [PATIENT_RACE_NAT_HI_ALL],
    [PATIENT_RACE_NAT_HI_GT3_IND],
    [PATIENT_RACE_WHITE_1],
    [PATIENT_RACE_WHITE_2],
    [PATIENT_RACE_WHITE_3],
    [PATIENT_RACE_WHITE_ALL],
    [PATIENT_RACE_WHITE_GT3_IND],
    [PATIENT_RECORD_STATUS],
    [PATIENT_SPEAKS_ENGLISH],
    [PATIENT_SSN],
    [PATIENT_STATE],
    [PATIENT_STATE_CODE],
    [PATIENT_STREET_ADDRESS_1],
    [PATIENT_STREET_ADDRESS_2],
    [PATIENT_UID],
    [PATIENT_UNK_ETHNIC_RSN],
    [PATIENT_WITHIN_CITY_LIMITS],
    [PATIENT_ZIP]
FROM [RDB_MODERN].[dbo].[D_PATIENT]
WHERE [PATIENT_LOCAL_ID] = 'PSN1000005000GA01'
AND PATIENT_LAST_CHANGE_TIME = '2026-04-10 20:26:11.770';

-- 1: EVENT_METRIC
SELECT
    [ADD_TIME],
    [ADD_USER_ID],
    [ADD_USER_NAME],
    [CASE_CLASS_CD],
    [CASE_CLASS_DESC_TXT],
    [CONDITION_CD],
    [CONDITION_DESC_TXT],
    [ELECTRONIC_IND],
    [INVESTIGATION_STATUS_CD],
    [INVESTIGATION_STATUS_DESC_TXT],
    [JURISDICTION_CD],
    [JURISDICTION_DESC_TXT],
    [LAST_CHG_TIME],
    [LAST_CHG_USER_ID],
    [LAST_CHG_USER_NAME],
    [LOCAL_ID],
    [LOCAL_PATIENT_ID],
    [PROGRAM_JURISDICTION_OID],
    [PROG_AREA_CD],
    [PROG_AREA_DESC_TXT],
    [RECORD_STATUS_CD],
    [RECORD_STATUS_DESC_TXT],
    [RECORD_STATUS_TIME],
    [STATUS_CD],
    [STATUS_DESC_TXT],
    [STATUS_TIME]
FROM [RDB_MODERN].[dbo].[EVENT_METRIC]
WHERE [LOCAL_PATIENT_ID] = 'PSN1000005000GA01';

-- 2: EVENT_METRIC_INC
SELECT
    [ADD_TIME],
    [ADD_USER_ID],
    [ADD_USER_NAME],
    [CASE_CLASS_CD],
    [CASE_CLASS_DESC_TXT],
    [CONDITION_CD],
    [CONDITION_DESC_TXT],
    [ELECTRONIC_IND],
    [INVESTIGATION_STATUS_CD],
    [INVESTIGATION_STATUS_DESC_TXT],
    [JURISDICTION_CD],
    [JURISDICTION_DESC_TXT],
    [LAST_CHG_TIME],
    [LAST_CHG_USER_ID],
    [LAST_CHG_USER_NAME],
    [LOCAL_ID],
    [LOCAL_PATIENT_ID],
    [PROGRAM_JURISDICTION_OID],
    [PROG_AREA_CD],
    [PROG_AREA_DESC_TXT],
    [RECORD_STATUS_CD],
    [RECORD_STATUS_DESC_TXT],
    [RECORD_STATUS_TIME],
    [STATUS_CD],
    [STATUS_DESC_TXT],
    [STATUS_TIME]
FROM [RDB_MODERN].[dbo].[EVENT_METRIC_INC]
WHERE [LOCAL_ID] = N'OBS1000005027GA01';

-- 3: MORBIDITY_REPORT_DATAMART
SELECT
    [AGE_REPORTED],
    [AGE_REPORTED_UNIT],
    [CASE_STATUS],
    [CONDITION_NAME],
    [DAYCARE],
    [DELIVERY_METHOD],
    [DIAGNOSIS_DATE],
    [DIE_FROM_ILLNESS],
    [EXTERNAL_IND],
    [FOOD_HANDLER],
    [FOOD_WATERBORNE_ILLNESS],
    [HEALTHCARE_ORGANIZATION],
    [HOSPITALIZED],
    [HOSPITAL_ADMIN_DATE],
    [HOSPITAL_DISCHARGE_DATE],
    [HOSPITAL_FAC_CITY],
    [HOSPITAL_FAC_NAME],
    [HOSPITAL_FAC_PHONE],
    [HOSPITAL_FAC_PHONE_EXT],
    [HOSPITAL_FAC_STATE],
    [HOSPITAL_FAC_STREET_ADDR_1],
    [HOSPITAL_FAC_STREET_ADDR_2],
    [HOSPITAL_FAC_ZIP],
    [ILLNESS_ONSET_DATE],
    [INVESTIGATION_CREATED_IND],
    [JURISDICTION_NAME],
    [LAB_GT3_CREATED_IND],
    [LAB_REPORT_DATE_1],
    [LAB_REPORT_DATE_2],
    [LAB_REPORT_DATE_3],
    [LAB_RESULT_COMMENTS_1],
    [LAB_RESULT_COMMENTS_2],
    [LAB_RESULT_COMMENTS_3],
    [MORBIDITY_REPORT_DATE],
    [MORBIDITY_REPORT_LOCAL_ID],
    [MORBIDITY_REPORT_TYPE],
    [MORB_REPORT_CREATED_BY],
    [MORB_REPORT_CREATE_DATE],
    [MORB_REPORT_LAST_UPDATED_BY],
    [MORB_REPORT_LAST_UPDATED_DATE],
    [MORB_RPT_COMMENTS],
    [NURSING_HOME],
    [OTHER_EPI],
    [PATIENT_CITY],
    [PATIENT_COUNTRY],
    [PATIENT_COUNTY],
    [PATIENT_CURRENT_SEX],
    [PATIENT_DECEASED_DATE],
    [PATIENT_DECEASED_INDICATOR],
    [PATIENT_DOB],
    [PATIENT_ETHNICITY],
    [PATIENT_FIRST_NAME],
    [PATIENT_GENERAL_COMMENTS],
    [PATIENT_LAST_NAME],
    [PATIENT_LOCAL_ID],
    [PATIENT_MARITAL_STATUS],
    [PATIENT_MIDDLE_NAME],
    [PATIENT_NAME_SUFFIX],
    [PATIENT_PHONE_EXT_HOME],
    [PATIENT_PHONE_EXT_WORK],
    [PATIENT_PHONE_NUMBER_HOME],
    [PATIENT_PHONE_NUMBER_WORK],
    [PATIENT_SSN],
    [PATIENT_STATE],
    [PATIENT_STREET_ADDRESS_1],
    [PATIENT_STREET_ADDRESS_2],
    [PATIENT_ZIP],
    [PH_RECEIVE_DT],
    [PREGNANT],
    [PROGRAM_AREA_DESCRIPTION],
    [PROGRAM_JURISDICTION_OID],
    [PROVIDER_CITY],
    [PROVIDER_FIRST_NAME],
    [PROVIDER_LAST_NAME],
    [PROVIDER_PHONE],
    [PROVIDER_PHONE_EXT],
    [PROVIDER_STATE],
    [PROVIDER_STREET_ADDR_1],
    [PROVIDER_STREET_ADDR_2],
    [PROVIDER_ZIP],
    [RACE_CALCULATED],
    [RACE_CALCULATED_DETAILS],
    [REPORTER_CITY],
    [REPORTER_FIRST_NAME],
    [REPORTER_LAST_NAME],
    [REPORTER_PHONE],
    [REPORTER_PHONE_EXT],
    [REPORTER_STATE],
    [REPORTER_STREET_ADDR_1],
    [REPORTER_STREET_ADDR_2],
    [REPORTER_ZIP],
    [REPORTING_FACILITY_UID],
    [REPORT_FAC_CITY],
    [REPORT_FAC_NAME],
    [REPORT_FAC_PHONE],
    [REPORT_FAC_PHONE_EXT],
    [REPORT_FAC_STATE],
    [REPORT_FAC_STREET_ADDR_1],
    [REPORT_FAC_STREET_ADDR_2],
    [REPORT_FAC_ZIP],
    [RESULTED_TEST_NAME_1],
    [RESULTED_TEST_NAME_2],
    [RESULTED_TEST_NAME_3],
    [RESULTED_TEST_NUMERIC_CONCAT_1],
    [RESULTED_TEST_NUMERIC_CONCAT_2],
    [RESULTED_TEST_NUMERIC_CONCAT_3],
    [RESULTED_TEST_RESULT_1],
    [RESULTED_TEST_RESULT_2],
    [RESULTED_TEST_RESULT_3],
    [RESULTED_TEST_TEXT_RESULT_1],
    [RESULTED_TEST_TEXT_RESULT_2],
    [RESULTED_TEST_TEXT_RESULT_3],
    [SPECIMEN_COLLECTION_DATE_1],
    [SPECIMEN_COLLECTION_DATE_2],
    [SPECIMEN_COLLECTION_DATE_3],
    [SPECIMEN_SOURCE_1],
    [SPECIMEN_SOURCE_2],
    [SPECIMEN_SOURCE_3],
    [TREATMENT_COMMENTS_1],
    [TREATMENT_COMMENTS_2],
    [TREATMENT_COMMENTS_3],
    [TREATMENT_CUSTOM_NAME_1],
    [TREATMENT_CUSTOM_NAME_2],
    [TREATMENT_CUSTOM_NAME_3],
    [TREATMENT_DATE_1],
    [TREATMENT_DATE_2],
    [TREATMENT_DATE_3],
    [TREATMENT_GT3_CREATED_IND],
    [TREATMENT_NAME_1],
    [TREATMENT_NAME_2],
    [TREATMENT_NAME_3]
FROM [RDB_MODERN].[dbo].[MORBIDITY_REPORT_DATAMART]
WHERE [PATIENT_LOCAL_ID] = 'PSN1000005000GA01';

-- 4: MORBIDITY_REPORT_EVENT
SELECT
	TOP 1
	[RECORD_STATUS_CD]
FROM
	[RDB_MODERN].[dbo].[MORBIDITY_REPORT_EVENT]
WHERE
	PATIENT_KEY IN (
		SELECT
			PATIENT_KEY
		FROM
			[RDB_MODERN].[dbo].[D_PATIENT]
		WHERE
			PATIENT_LOCAL_ID = 'PSN1000005000GA01'
	);

-- 5: MORBIDITY_REPORT
SELECT
    [DAYCARE_IND],
    [DIAGNOSIS_DT],
    [DIE_FROM_ILLNESS_IND],
    [ELECTRONIC_IND],
    [FOOD_HANDLER_IND],
    [HEALTHCARE_ORG_ASSOCIATE_IND],
    [HOSPITALIZED_IND],
    [HSPTL_ADMISSION_DT],
    [JURISDICTION_CD],
    [JURISDICTION_NM],
    [MORB_RPT_CREATE_BY],
    [MORB_RPT_KEY],
    [MORB_RPT_LAST_UPDATE_BY],
    [MORB_RPT_LAST_UPDATE_DT],
    [MORB_RPT_LOCAL_ID],
    [MORB_RPT_OID],
    [MORB_RPT_OTHER_SPECIFY],
    [MORB_RPT_SHARE_IND],
    [MORB_RPT_TYPE],
    [MORB_RPT_UID],
    [NURSING_HOME_ASSOCIATE_IND],
    [PH_RECEIVE_DT],
    [PREGNANT_IND],
    [RDB_LAST_REFRESH_TIME],
    [RECORD_STATUS_CD],
    [SUSPECT_FOOD_WTRBORNE_ILLNESS]
FROM [RDB_MODERN].[dbo].[MORBIDITY_REPORT]
WHERE [MORB_RPT_LOCAL_ID] = 'OBS1000005027GA01';

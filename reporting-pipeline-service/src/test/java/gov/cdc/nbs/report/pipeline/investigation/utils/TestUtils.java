...
public class TestUtils {
  ...
  public static final String FILE_PATH_PREFIX = "rawDataFiles/investigation/";
  ...
  public static Investigation constructInvestigation(Long investigationUid) {
    ...
    investigation.setInvestigationConfirmationMethod(
        readFileData(FILE_PATH_PREFIX + "ConfirmationMethod.json"));
    investigation.setInvestigationObservationIds(
        readFileData(FILE_PATH_PREFIX + "InvestigationObservationIds.json"));
    investigation.setOrganizationParticipations(
        readFileData(FILE_PATH_PREFIX + "OrganizationParticipations.json"));
    investigation.setPersonParticipations(
        readFileData(FILE_PATH_PREFIX + "PersonParticipations.json"));
    investigation.setInvestigationCaseAnswer(
        readFileData(FILE_PATH_PREFIX + "InvestigationCaseAnswers.json"));
    investigation.setInvestigationNotifications(
        readFileData(FILE_PATH_PREFIX + "InvestigationNotification.json"));
    investigation.setInvestigationCaseCnt(readFileData(FILE_PATH_PREFIX + "CaseCountInfo.json"));
    investigation.setInvestigationCaseManagement(
        readFileData(FILE_PATH_PREFIX + "CaseManagement.json"));
  ...
  public static InvestigationReporting constructInvestigationReporting(Long investigationUid) {
    ...
    reporting.setInvestigatorId(
        32143250L); // PersonParticipations.json, entity_id for type_cd=InvestgrOfPHC
    reporting.setPhysicianId(
        14253651L); // PersonParticipations.json, entity_id for type_cd=PhysicianOfPHC
    reporting.setPatientId(
        321432537L); // PersonParticipations.json, entity_id for type_cd=SubjOfPHC
    reporting.setPersonAsReporterUid(
        10003004L); // PersonParticipations.json, entity_id for type_cd=PerAsReporterOfPHC
    reporting.setOrganizationId(
        34865315L); // OrganizationParticipations.json, entity_id for type_cd=OrgAsReporterOfPHC
    reporting.setHospitalUid(
        30303034L); // OrganizationParticipations.json, entity_id for type_cd=HospOfADT
    reporting.setChronicCareFacUid(
        31096761L); // OrganizationParticipations.json, entity_id for type_cd=ChronicCareFac
    reporting.setDaycareFacUid(
        30303007L); // OrganizationParticipations.json, entity_id for type_cd=DaycareFac
    reporting.setInvStateCaseId("12-345-STA"); // ActIds.json, root_extension_txt for type_cd=STATE
    reporting.setCityCountyCaseNbr(
        "12-345-CTY"); // ActIds.json, root_extension_txt for type_cd=CITY
    reporting.setLegacyCaseId("12-345-LGY"); // ActIds.json, root_extension_txt for type_cd=LEGACY
    reporting.setPhcInvFormId(
        10638298L); // InvestigationObservationIds.json, source_act_uid for act_type_cd=PHCInvForm
    reporting.setRdbTableNameList(
        "D_INV_CLINICAL,D_INV_PLACE_REPEAT,D_INV_ADMINISTRATIVE"); // InvestigationCaseAnswers.json,
    // rdb_table_nm
  ...
  public static NotificationUpdate constructNotificationUpdate(Long notificationUid) {
    ...
    notification.setInvestigationNotifications(
        readFileData(FILE_PATH_PREFIX + "InvestigationNotification.json"));
    return notification;
  }
  ...
  public static InterviewReporting constructInvestigationInterview(
      Long interviewUid, Long batchId) {
  ...
  public static InterviewAnswer constructInvestigationInterviewAnswer(
      Long interviewUid, Long batchId) {

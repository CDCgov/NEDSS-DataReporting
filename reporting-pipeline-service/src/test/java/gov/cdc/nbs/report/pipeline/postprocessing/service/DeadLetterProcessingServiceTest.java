...
class DeadLetterProcessingServiceTest {
  ...
  @Mock private DeadLetterLogRepository deadLetterLogRepository;

  @InjectMocks private DeadLetterProcessingService deadLetterProcessingService;
  ...
  void testConstructor() {
    ...
    assertNotNull(
        ReflectionTestUtils.getField(deadLetterProcessingService, "deadLetterLogRepository"));
  }
  ...
  void testHandlingDeadLetter_RepositoryException() {
    ...
    assertTrue(
        logs.stream().anyMatch(log -> log.getMessage().contains("Database connection failed")));
  }
  ...
  void testKafkaListenerAnnotation() throws Exception {
    ...
    Method method =
        DeadLetterProcessingService.class.getMethod(
            "handlingDeadLetter",
            String.class,
            String.class,
            Long.class,
    ...
    assertTrue(
        method.isAnnotationPresent(org.springframework.kafka.annotation.KafkaListener.class));

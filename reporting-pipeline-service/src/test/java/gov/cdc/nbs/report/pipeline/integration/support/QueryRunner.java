      ...
      try {
        ...
        if (result.isEmpty()) {
          ...
          try {
              ...
              System.err.println("RECENT JOB_FLOW_LOG entries: " + client.sql("SELECT TOP 20 Status_Type, Step_Name, row_count, Error_Description FROM RDB_MODERN.DBO.JOB_FLOW_LOG ORDER BY record_id DESC").query().listOfRows());
              System.err.println("F_PAGE_CASE count: " + client.sql("SELECT COUNT(*) as cnt FROM RDB_MODERN.DBO.F_PAGE_CASE").query().listOfRows());
              System.err.println("INVESTIGATION count: " + client.sql("SELECT COUNT(*) as cnt FROM RDB_MODERN.DBO.INVESTIGATION").query().listOfRows());

package gov.cdc.nbs.report.pipeline.util.json;
...
class CustomJsonGeneratorImplTest {
  ...
  void testGenerateStringJson() {
    ...
    /**
     * { "schema":{ "type":"struct", "fields":[{"type":"string","optional":true,"field":"attr1"}] },
  ...
  void testGenerateStringJson_ComplexObject() {
    ComplexTestBean testBean =
        new ComplexTestBean("test", 42, 1234L, Instant.ofEpochSecond(1001), "annotated");
    String jsonVal = jsonGenerator.generateStringJson(testBean);
  ...
  void testGenerateStringJson_WithOverrideKeys() {
    ...
    assertTrue(
        jsonVal.contains("\"optional\":false") && jsonVal.contains("\"field\":\"string_field\""));
    assertTrue(
        jsonVal.contains("\"optional\":false") && jsonVal.contains("\"field\":\"custom_name\""));
    assertTrue(
        jsonVal.contains("\"optional\":true") && jsonVal.contains("\"field\":\"int_field\""));
  }

  @Test
  void testGenerateStringJson_NullModel() {
    RuntimeException ex =
        assertThrows(RuntimeException.class, () -> jsonGenerator.generateStringJson(null));

    assertTrue(
        ex.getMessage().contains("Cannot invoke \"Object.getClass()\" because \"model\" is null"));
  }

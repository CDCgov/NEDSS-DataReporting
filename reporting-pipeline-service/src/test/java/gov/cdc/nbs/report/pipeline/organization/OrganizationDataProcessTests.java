...
class OrganizationDataProcessTests {
    ...
    Object expected =
        switch (type) {
          case ORGANIZATION_REPORTING ->
              deserializePayload(
                  objectMapper
                      .readTree(readFileData("rawDataFiles/organization/OrgReporting.json"))
                      .path("payload")
                      .toString(),
                  OrganizationReporting.class);
          case ORGANIZATION_ELASTIC_SEARCH ->
              deserializePayload(
                  objectMapper
                      .readTree(readFileData("rawDataFiles/organization/OrgElastic.json"))
                      .path("payload")
                      .toString(),
  ...
  void testInvalidDataDeserialization() {
    ...
    OrganizationReporting orgReporting =
        deserializePayload(invalidJson, OrganizationReporting.class);

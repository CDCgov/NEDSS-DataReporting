...
import gov.cdc.nbs.report.pipeline.person.model.dto.provider.ProviderKey;
...
class PersonDataPostProcessingTests {
  ...
  void testPatientNameTransformation() {
    ...
    PatientReporting expected =
        PatientReporting.builder().patientUid(10000001L).aliasNickname("XEZD6SLFPRUJQGA52").build();
  ...
  void testProviderNameTransformation() {
    ...
    ProviderReporting expected = ProviderReporting.builder().providerUid(10000001L).build();
  ...
  private <T> T getTransformed(PersonTypeExt type) throws JsonProcessingException {
    return deserializePayload(
        objectMapper.readTree(readFileData(type.getFileName())).path("payload").toString(),
        type.getType().getClazz());
  }
  ...
  private enum PersonTypeExt {
    ...
    PATIENT_ELASTIC_SEARCH(
        PersonType.PATIENT_ELASTIC_SEARCH, FILE_PREFIX_PAT + "PatientElastic.json"),
    PROVIDER_REPORTING(PersonType.PROVIDER_REPORTING, FILE_PREFIX_PRV + "ProviderReporting.json"),
    PROVIDER_ELASTIC_SEARCH(
        PersonType.PROVIDER_ELASTIC_SEARCH, FILE_PREFIX_PRV + "ProviderElasticSearch.json");

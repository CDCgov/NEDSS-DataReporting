...
import static gov.cdc.etldatapipeline.postprocessing.service.Entity.LDF_VACCINE_PREVENT_DISEASES;
import static gov.cdc.etldatapipeline.postprocessing.service.ProcessDatamartData.*;
...
class PostProcessingServiceRetryTest {
  ...
  @Mock private static InvestigationRepository investigationRepositoryMock;
  ...
  void testProcessRetryEntity() {
    ...
    verify(postProcRepositoryMock, never())
        .executeStoredProcForBackfill(
            Entity.PATIENT.getEntityName().toUpperCase(),
            patientUid,
            batchId,
            errorMsg,
            STATUS_READY,
  ...
  void testProcessLdfVaccine() {
    ...
    String phcUid = "123";
    when(investigationRepositoryMock.executeStoredProcForLdfVacPreventDiseasesDatamart(phcUid))
        .thenThrow(new RuntimeException(errorMsg));
  ...
  void testProcessRetryMultiId() {
    ...
    when(postProcRepositoryMock.executeStoredProcForInvSummaryDatamart("123", "124", "130"))
        .thenThrow(new RuntimeException(errorMsg));
    postProcessingServiceMock.processDatamartIds();
  ...
  void testProcessBackfillEvent() {
    ...
    postProcessingServiceMock.processRetryCache();
    verify(postProcRepositoryMock)
        .executeStoredProcForBackfill(null, null, 123123L, errorMsg, STATUS_SUSPENDED, 0);

    datamartProcessor.processRetryCache();
    verify(postProcRepositoryMock)
        .executeStoredProcForBackfill(null, null, 123456L, errorMsg, STATUS_SUSPENDED, 0);

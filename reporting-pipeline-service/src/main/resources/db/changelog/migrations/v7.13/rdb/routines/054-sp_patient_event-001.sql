...
BEGIN
    ...
    BEGIN TRY
        ...
        ;WITH ranked_country AS (
            SELECT code, code_short_desc_txt,
                   RANK() OVER (PARTITION BY code ORDER BY nbs_uid DESC) rnk
        ...
        ;WITH ranked_suffix AS (
            SELECT cvg.code, cvg.code_short_desc_txt,
                   ROW_NUMBER() OVER (PARTITION BY cvg.code ORDER BY cvg.code_short_desc_txt) rn
            FROM nbs_odse.dbo.nbs_question nq WITH (NOLOCK)
                     JOIN nbs_srte.dbo.codeset cs WITH (NOLOCK) ON cs.code_set_group_id = nq.code_set_group_id
                     JOIN nbs_srte.dbo.code_value_general cvg WITH (NOLOCK) ON cvg.code_set_nm = cs.code_set_nm
        ...
        ;WITH ranked_gender AS (
            SELECT code, code_short_desc_txt,
                   ROW_NUMBER() OVER (PARTITION BY code ORDER BY code_short_desc_txt) rn
            FROM nbs_srte.dbo.code_value_general WITH (NOLOCK)
        ...
        ranked_users AS (
            SELECT au.NEDSS_ENTRY_ID,
                   CAST((RTRIM(LTRIM(au.user_last_nm)) + ', ' + RTRIM(LTRIM(au.user_first_nm))) AS VARCHAR(150)) AS user_full_name,
                   ROW_NUMBER() OVER (PARTITION BY au.NEDSS_ENTRY_ID ORDER BY au.last_chg_time DESC) rn
            FROM NBS_ODSE.dbo.Auth_user au WITH (NOLOCK)
                     JOIN user_ids ui ON ui.nedss_entry_id = au.NEDSS_ENTRY_ID
        )
        SELECT NEDSS_ENTRY_ID, user_full_name
        INTO #user_names
        FROM ranked_users
        WHERE rn = 1;
        ...
        FROM #filtered_person fp
                 ...
                 LEFT OUTER JOIN nbs_odse.dbo.Postal_locator pl WITH (NOLOCK) ON pl.postal_locator_uid = elp.locator_uid
                 ...
                 LEFT OUTER JOIN #country_code_latest ccl ON ccl.code = pl.cntry_cd;
        ...
        FROM #filtered_person fp
                 JOIN nbs_odse.dbo.Entity_locator_participation elp WITH (NOLOCK)
                      ON elp.entity_uid = fp.person_uid
                          ...
                          AND elp.record_status_cd = 'ACTIVE'
                 JOIN nbs_odse.dbo.Tele_locator tl WITH (NOLOCK) ON tl.tele_locator_uid = elp.locator_uid
        WHERE tl.phone_nbr_txt IS NOT NULL OR tl.email_address IS NOT NULL;
        ...
        INTO #person_name
        FROM #filtered_person fp
                 JOIN nbs_odse.dbo.person_name pn WITH (NOLOCK) ON pn.person_uid = fp.person_uid
                 LEFT OUTER JOIN #suffix_codes sfx ON sfx.code = NULLIF(pn.nm_suffix, '')
        ...
        SELECT
            ...
            trt.race_amer_ind_1, trt.race_amer_ind_2, trt.race_amer_ind_3, trt.race_amer_ind_gt3_ind, trt.race_amer_ind_all,
            trt.race_asian_1, trt.race_asian_2, trt.race_asian_3, trt.race_asian_gt3_ind, trt.race_asian_all,
            trt.race_black_1, trt.race_black_2, trt.race_black_3, trt.race_black_gt3_ind, trt.race_black_all,
            trt.race_nat_hi_1, trt.race_nat_hi_2, trt.race_nat_hi_3, trt.race_nat_hi_gt3_ind, trt.race_nat_hi_all,
            trt.race_white_1, trt.race_white_2, trt.race_white_3, trt.race_white_gt3_ind, trt.race_white_all,
            trt.race_all
        INTO #person_race
        FROM #filtered_person fp
                 JOIN nbs_odse.dbo.person_race pr WITH (NOLOCK) ON pr.person_uid = fp.person_uid
                 LEFT OUTER JOIN nbs_srte.dbo.race_code src WITH (NOLOCK) ON pr.race_cd = src.code
                 LEFT OUTER JOIN #temp_race_table trt ON trt.patient_uid_race_out = fp.person_uid;
        ...
        FROM #filtered_person fp
                 JOIN nbs_odse.dbo.entity_id ei WITH (NOLOCK) ON ei.entity_uid = fp.person_uid;
        ...
        FROM #filtered_person p
                 ...
                 -- Preferred-gender description, resolved from the
                 -- pre-built NBS_STD_GENDER_PARPT lookup rather than a
                 -- per-row code_value_general join.
                 LEFT JOIN #gender_codes gc ON gc.code = NULLIF(p.preferred_gender_cd, '')

                 -- Display name of the user who created the record,
                 -- resolved from the pre-built Auth_user lookup.
                 LEFT JOIN #user_names aun ON aun.NEDSS_ENTRY_ID = p.add_user_id

                 -- Display name of the user who last modified the record,
                 -- resolved from the pre-built Auth_user lookup.
                 LEFT JOIN #user_names lun ON lun.NEDSS_ENTRY_ID = p.last_chg_user_id

                 -- =========================================================
                 -- Per-person nested JSON payload. Each sub-select builds
                 -- one JSON array (via FOR JSON PATH) for a related child
                 -- entity set, pulling from the pre-joined #person_*
                 -- staging tables, all combined into a single "nested" row
                 -- so they can be selected as individual JSON columns above.
                 -- =========================================================
                 OUTER apply (SELECT *
                              FROM
                                  -- ADDRESS: home and birth postal addresses for this patient.
                                  (SELECT (SELECT elp_cd            AS [addr_elp_cd],
                                  ...
                                  (SELECT (SELECT tl_uid    AS [ph_tl_uid],
                                                  elp_cd    AS [ph_elp_cd],
                                  ...
                                  (SELECT (SELECT tl_uid       AS [email_tl_uid],
                                  ...
                                  (SELECT (SELECT person_uid       AS [pn_person_uid],
                                  ...
                                  (SELECT (SELECT person_uid          AS [pr_person_uid],
                                                  race_cd             AS [raceCd],
                                  ...
                                  (SELECT (SELECT person_uid           AS [entity_uid],
                                                  type_cd              AS [typeCd],

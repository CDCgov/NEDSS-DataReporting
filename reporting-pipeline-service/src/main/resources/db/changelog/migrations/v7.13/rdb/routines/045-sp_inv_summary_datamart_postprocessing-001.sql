IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_inv_summary_datamart_postprocessing]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_inv_summary_datamart_postprocessing]
END
GO 

CREATE PROCEDURE dbo.sp_inv_summary_datamart_postprocessing
...
----------------------------------------------------------------------------
        ...
        FROM dbo.NOTIFICATION_EVENT with (nolock)
                 INNER JOIN DBO.NOTIFICATION with (nolock)
                 ...
                 INNER JOIN DBO.INVESTIGATION with (nolock)
                            ON INVESTIGATION.INVESTIGATION_KEY = NOTIFICATION_EVENT.INVESTIGATION_KEY
        WHERE (
            (
            ...
            NOTIFICATION.NOTIFICATION_KEY in (select NOTIFICATION_KEY
                                              from dbo.NOTIFICATION with (nolock)
                                                       inner join dbo.nrt_notification_key k
                                                                  on k.d_notification_key = NOTIFICATION.NOTIFICATION_KEY
...
----------------------------------------------------------------------------
        ...
        if (@COUNTSTD > 0) ------------------STD CASES
            ...
            BEGIN
                ...
                SELECT DISTINCT I.INVESTIGATION_KEY                               AS 'INVESTIGATION_KEY',---1
                                ...
                                CASE
                                    WHEN COALESCE(GC.PATIENT_KEY, 0) > 1 then GC.PATIENT_KEY
                                    WHEN COALESCE(CC.PATIENT_KEY, 0) > 1 then CC.PATIENT_KEY
                                    WHEN COALESCE(MC.PATIENT_KEY, 0) > 1 then MC.PATIENT_KEY
                                    WHEN COALESCE(RC.PATIENT_KEY, 0) > 1 then RC.PATIENT_KEY
                                    WHEN COALESCE(HC.PATIENT_KEY, 0) > 1 then HC.PATIENT_KEY
                                    WHEN COALESCE(BC.PATIENT_KEY, 0) > 1 then BC.PATIENT_KEY
                                    WHEN COALESCE(PC.PATIENT_KEY, 0) > 1 then PC.PATIENT_KEY
                                    WHEN COALESCE(F_TB.PERSON_KEY, 0) > 1 then F_TB.PERSON_KEY
                                    WHEN COALESCE(F_PAGE.PATIENT_KEY, 0) > 1 then F_PAGE.PATIENT_KEY
                                    WHEN COALESCE(F_VAR.PERSON_KEY, 0) > 1 then F_VAR.PERSON_KEY
                                    WHEN COALESCE(F_STD.PATIENT_KEY, 0) > 1 then F_STD.PATIENT_KEY---STD
                                    ELSE NULL END                                 AS 'PATIENT_KEY',
                                CASE
                                    WHEN COALESCE(GC.PHYSICIAN_KEY, 0) > 1 then GC.PHYSICIAN_KEY
                                    WHEN COALESCE(CC.PHYSICIAN_KEY, 0) > 1 then CC.PHYSICIAN_KEY
                                    WHEN COALESCE(MC.PHYSICIAN_KEY, 0) > 1 then MC.PHYSICIAN_KEY
                                    WHEN COALESCE(RC.PHYSICIAN_KEY, 0) > 1 then RC.PHYSICIAN_KEY
                                    WHEN COALESCE(HC.PHYSICIAN_KEY, 0) > 1 then HC.PHYSICIAN_KEY
                                    WHEN COALESCE(BC.PHYSICIAN_KEY, 0) > 1 then BC.PHYSICIAN_KEY
                                    WHEN COALESCE(PC.PHYSICIAN_KEY, 0) > 1 then PC.PHYSICIAN_KEY
                                    WHEN COALESCE(F_TB.PHYSICIAN_KEY, 0) > 1 then F_TB.PHYSICIAN_KEY
                                    WHEN COALESCE(F_VAR.PHYSICIAN_KEY, 0) > 1 then F_VAR.PHYSICIAN_KEY
...
--                    Cast (NULL as  bigint)               AS  'PHYSICIAN_KEY'---46
                FROM [dbo].[INVESTIGATION] I with (nolock)
                         ...
                         FULL JOIN [dbo].F_PAGE_CASE F_PAGE with (nolock)
                                   ON F_PAGE.INVESTIGATION_KEY = I.INVESTIGATION_KEY
                         FULL JOIN [dbo].[F_STD_PAGE_CASE] F_STD with (nolock)
                                   ON F_STD.INVESTIGATION_KEY = I.INVESTIGATION_KEY--STD
            ...
            begin
                ...
                SELECT DISTINCT I.INVESTIGATION_KEY                               AS 'INVESTIGATION_KEY',---1
                                ...
                                CASE
                                    ...
                                    WHEN COALESCE(F_PAGE.PATIENT_KEY, 0) > 1 then F_PAGE.PATIENT_KEY
                                    WHEN COALESCE(F_VAR.PERSON_KEY, 0) > 1 then F_VAR.PERSON_KEY
                                    ELSE NULL END                                 AS 'PATIENT_KEY',---45,
                                CASE
                                    WHEN COALESCE(GC.PHYSICIAN_KEY, 0) > 1 then GC.PHYSICIAN_KEY
                                    WHEN COALESCE(CC.PHYSICIAN_KEY, 0) > 1 then CC.PHYSICIAN_KEY
                                    WHEN COALESCE(MC.PHYSICIAN_KEY, 0) > 1 then MC.PHYSICIAN_KEY
...
----------------------------------------------------------------------------
        ...
        SELECT INVESTIGATION_KEY,
               CONFIRMATION_DT,
                 LEFT(STRING_AGG(CAST(LTRIM(RTRIM(CONFIRMATION_METHOD_DESC)) AS NVARCHAR(MAX)), ' | '), 4000) AS CONFIRMATION_METHOD
...
----------------------------------------------------------------------------
        ...
        With CTE as (SELECT A.*,
                     ...
                     FROM #TMP_PATIENT_DETAILS A
                              ...
                              LEFT OUTER JOIN [dbo].[NOTIFICATION_EVENT] NOT_EVENT with (nolock)
                              ...
                              LEFT OUTER JOIN [dbo].[RDB_DATE] RDB_DATE_SENT with (nolock)
                                              ON NOT_EVENT.NOTIFICATION_SENT_DT_KEY = RDB_DATE_SENT.DATE_KEY)
        SELECT *
        INTO #TMP_S_INV_SUMM_DATAMART_INIT
        from CTE
        where rn = 1;
...
---------------------------------------------------------12. Create table TMP_InvLab
        ...
        WHERE (L.LAB_TEST_KEY IN (SELECT LAB_TEST_KEY
                                  FROM dbo.LAB_TEST
                                  WHERE LAB_RPT_UID IN (SELECT value FROM STRING_SPLIT(@obs_uids, ','))))
...
-----------------------------------------------------------------16. Create Table TMP_CASE_LAB_DATAMART_MODIFIED---------------------------------
        ...
        select A.*
             , CASE
                   WHEN A.NOTIFICATION_STATUS IS NOT NULL THEN A.INVESTIGATION_CREATED_BY
                   ELSE NULL END                            AS NOTIFICATION_SUBMITTER         ---42
             --,A.INVESTIGATION_LAST_UPDTD_DATE AS NOTIFICATION_LAST_UPDATED_DATE	---43
             , CASE
                   WHEN A.NOTIFICATION_STATUS IS NOT NULL THEN coalesce(A.NOTIFICATION_LAST_CHANGE_TIME,
        ...
        FROM #TMP_S_INV_SUMM_DATAMART_INIT A
                 ...
                 LEFT OUTER JOIN #TMP_CASE_LAB_DATAMART_MODIFIED S with (nolock)
                                 ON A.INVESTIGATION_KEY = S.INVESTIGATION_KEY;
...
----------------------------------------------------------------------------
        ...
        INSERT INTO dbo.INV_SUMM_DATAMART( [INVESTIGATION_KEY]
                                         , [PATIENT_KEY]
                                         , [PATIENT_LOCAL_ID]
...
----------------------------------------------------------------------------
        ...
        FROM dbo.INV_SUMM_DATAMART ISD
                 LEFT OUTER JOIN [dbo].[CASE_LAB_DATAMART] CLD with (nolock)
                                 ON ISD.INVESTIGATION_KEY = CLD.INVESTIGATION_KEY
                 JOIN #TMP_CASE_LAB_DATAMART_MODIFIED TCLDM with (nolock)
                      ON ISD.INVESTIGATION_KEY = TCLDM.INVESTIGATION_KEY;

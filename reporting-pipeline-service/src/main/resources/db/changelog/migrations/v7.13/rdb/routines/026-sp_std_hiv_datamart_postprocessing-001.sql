IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_std_hiv_datamart_postprocessing]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_std_hiv_datamart_postprocessing]
END
GO 

CREATE PROCEDURE dbo.sp_std_hiv_datamart_postprocessing
    @phc_id nvarchar(max),
    @debug bit = 'false'
AS
...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM [DBO].[INV_HIV] ih
                 INNER JOIN [DBO].[F_STD_PAGE_CASE] PC  with(nolock) on ih.INVESTIGATION_KEY = PC.INVESTIGATION_KEY
                 INNER JOIN #tmp_investigation inv ON inv.INVESTIGATION_KEY= PC.INVESTIGATION_KEY
                 LEFT OUTER JOIN [DBO].[D_INV_HIV] dih  with(nolock) ON dih.D_INV_HIV_KEY = PC.D_INV_HIV_KEY
        WHERE
        ...
        FROM
            [DBO].[F_STD_PAGE_CASE] PC with(nolock)
                LEFT OUTER JOIN
            [DBO].[D_INV_HIV] dih with(nolock)
        ...
        SET
          ...
          -- using CD column correctly from CM
        ...
        FROM [dbo].[STD_HIV_DATAMART] shd
                 INNER JOIN dbo.F_STD_PAGE_CASE PC with(nolock) ON  shd.INVESTIGATION_KEY = PC.INVESTIGATION_KEY
                 INNER JOIN #tmp_investigation INV ON INV.INVESTIGATION_KEY = PC.INVESTIGATION_KEY
                 LEFT JOIN dbo.condition COND with(nolock) ON COND.CONDITION_KEY = PC.CONDITION_KEY
            ...
            AND SURV.PROVIDER_KEY != 1
                 LEFT JOIN (SELECT IXC.INVESTIGATION_KEY, DV.IX_DATE
                            FROM dbo.F_INTERVIEW_CASE IXC with(nolock)

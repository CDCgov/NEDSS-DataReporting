IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_nrt_var_pam_ldf_postprocessing]')
                                      AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
    BEGIN
        DROP PROCEDURE [dbo].[sp_nrt_var_pam_ldf_postprocessing]
    END
GO

CREATE PROCEDURE dbo.sp_nrt_var_pam_ldf_postprocessing
...
--------------------------------------------------------------------------------------------------------
        ...
        SELECT
            ...
            LEFT(STRING_AGG(CAST(ANSWER_TXT AS NVARCHAR(MAX)), ' | ') WITHIN GROUP (ORDER BY ANSWER_TXT), 2000) AS ANSWER_TXT
...
--------------------------------------------------------------------------------------------------------
        BEGIN TRANSACTION
            SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
            SET @PROC_STEP_NAME = 'GENERATING  TABLE';
...
--------------------------------------------------------------------------------------------------------
        SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET @PROC_STEP_NAME = 'GENERATING ordered_columns';

        DECLARE @ordered_columns NVARCHAR(MAX) = '';

        SELECT @ordered_columns = STRING_AGG(CAST(QUOTENAME(COLUMN_NAME) AS NVARCHAR(MAX)), ',') WITHIN GROUP (ORDER BY ORDINAL_POSITION)
...
--------------------------------------------------------------------------------------------------------
        ...
        BEGIN TRANSACTION
            SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
            SET @PROC_STEP_NAME = 'Update dbo.VAR_PAM_LDF';
        ...
        INNER JOIN dbo.VAR_PAM_LDF tgt  with (nolock)
            ...
            if @debug = 'true' print @Update_sql;
...
--------------------------------------------------------------------------------------------------------
        ...
        BEGIN TRANSACTION
            SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
            SET @PROC_STEP_NAME = 'Inserting to dbo.VAR_PAM_LDF';
   ...
   on t.INVESTIGATION_KEY = tgt.INVESTIGATION_KEY and
        ...
        where tgt.VAR_PAM_UID is null;';
            ...
            if @debug = 'true' print @Insert_sql;

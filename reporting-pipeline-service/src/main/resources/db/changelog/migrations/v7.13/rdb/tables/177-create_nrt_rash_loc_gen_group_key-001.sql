...
IF EXISTS (SELECT 1 FROM sysobjects WHERE name = 'nrt_rash_loc_gen_group_key' and xtype = 'U')
    BEGIN
        IF NOT EXISTS(SELECT 1
                      FROM sys.columns
                      WHERE Name = N'created_dttm' AND Object_ID = Object_ID(N'nrt_rash_loc_gen_group_key'))
            BEGIN
        ...
        IF NOT EXISTS(SELECT 1
                      FROM sys.columns
                      WHERE Name = N'updated_dttm' AND Object_ID = Object_ID(N'nrt_rash_loc_gen_group_key'))
            BEGIN
                ALTER TABLE dbo.nrt_rash_loc_gen_group_key
...
IF NOT EXISTS(SELECT 1 FROM sys.objects WHERE type = 'PK' AND parent_object_id = OBJECT_ID('dbo.nrt_rash_loc_gen_group_key'))
    BEGIN
        ALTER TABLE dbo.nrt_rash_loc_gen_group_key

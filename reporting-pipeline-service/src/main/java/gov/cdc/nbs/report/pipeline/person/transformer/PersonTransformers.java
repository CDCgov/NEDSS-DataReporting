...
public class PersonTransformers {
  ...
  public String buildProviderKey(ProviderSp p) {
    return jsonGenerator.generateStringJson(
        ProviderKey.builder().providerUid(p.getPersonUid()).build());
  }

  public String buildUserKey(AuthUser u) {
    return jsonGenerator.generateStringJson(
        AuthUserKey.builder().authUserUid(u.getAuthUserUid()).build(), "auth_user_uid");
  }
  ...
  public PersonExtendedProps processData(
    ...
    if (personType == PersonType.PATIENT_REPORTING
        || personType == PersonType.PATIENT_ELASTIC_SEARCH) {
  ...
  public PatientReporting buildPatientReporting(PatientSp p) {
    return PatientReporting.builder()
        ...
        .deceasedIndCd(p.getDeceasedIndCd())

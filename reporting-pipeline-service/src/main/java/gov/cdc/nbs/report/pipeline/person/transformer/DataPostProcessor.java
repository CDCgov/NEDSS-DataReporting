...
import gov.cdc.nbs.report.pipeline.person.model.dto.persondetail.*;
...
public class DataPostProcessor {
  /**
   * 1. For patient name_use_cd = {L, AL} 2. For providers name_use_cd = {L} - For every name array,
   * get the json node with the max patient_uid and person_seq_num
   *
  ...
  public <T extends PersonExtendedProps> void processPersonName(String name, T pf) {
    if (!ObjectUtils.isEmpty(name)) {
      ...
      if (pf.getClass() == ProviderReporting.class
          || pf.getClass() == ProviderElasticSearch.class) {
        nameUseCds = List.of(NameUseCd.LEGAL).toArray(NameUseCd[]::new);
      }
      Arrays.stream(nameUseCds)
          .forEach(
              cd -> {
                TreeMap<Long, List<Name>> nameMap =
                    Arrays.stream(requireNonNull(deserializePayload(name, Name[].class)))
                        ...
                        .filter(
                            pName ->
                                ObjectUtils.isEmpty(pName.getNmUseCd())
                                    || pName.getNmUseCd().equals(cd.getVal()))
                        // Sort by the getPersonUid and collect all the entries with max PersonUid
                        // to a List
                        .collect(groupingBy(Name::getPersonUid, TreeMap::new, toList()));
                // Get the last entry which is the max PersonUid
                if (!nameMap.isEmpty()) {
                  nameMap.lastEntry().getValue().stream()
                          ...
                          Comparator.comparing(
                              n -> {
                                try {
                                  ...
                                  log.warn(
                                      "Invalid person name sequence number: {}",
                                      n.getPersonNmSeq());
                                  return 0;
                                }
                              }))
    ...
    if (!ObjectUtils.isEmpty(address)) {
      ...
      if (pf.getClass() == PatientReporting.class || pf.getClass() == PatientElasticSearch.class) {
        ...
        addrData.stream()
            ...
            .max(Comparator.comparing(Address::getPostalLocatorUid))
            .ifPresent(n -> n.updatePerson(pf));
      } else if (pf.getClass() == ProviderReporting.class
          || pf.getClass() == ProviderElasticSearch.class) {
        addrData.stream()

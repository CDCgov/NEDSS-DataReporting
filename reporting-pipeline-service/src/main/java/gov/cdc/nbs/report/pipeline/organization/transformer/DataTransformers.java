...
public class DataTransformers {
  ...
  public String buildOrganizationKey(OrganizationSp p) {
    return jsonGenerator.generateStringJson(
        OrganizationKey.builder().organizationUid(p.getOrganizationUid()).build());
  }

  public String buildPlaceKey(Place p) {
    return jsonGenerator.generateStringJson(
        PlaceKey.builder().placeUid(p.getPlaceUid()).build(), PLACE_UID);
  }

  public String buildPlaceTeleKey(PlaceTele p) {
    return jsonGenerator.generateStringJson(
        PlaceTeleKey.builder().placeTeleLocatorUid(p.getPlaceTeleLocatorUid()).build(),
        PLACE_TL_UID);
  }

  public String processData(OrganizationSp organizationSp, OrganizationType organizationType) {
    return jsonGenerator.generateStringJson(
        buildTransformedObject(organizationSp, organizationType));
  }
  ...
  public Object buildTransformedObject(
      OrganizationSp organizationSp, OrganizationType organizationType) {
  ...
  private PlaceReporting buildPlaceReporting(Place place) {
    PlaceReporting placeRep =
        PlaceReporting.builder()

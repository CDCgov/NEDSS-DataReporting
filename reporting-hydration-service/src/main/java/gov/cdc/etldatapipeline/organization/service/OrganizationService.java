...
/**
 * Service class for processing Organization and Place-related change events in the Real Time
 * Reporting (RTR) pipeline. This service handles the "hydration" of data for Organizations and
 * Places by consuming Kafka events from transactional source topics, transforming them, and
 * producing them to reporting topics.
 *
 * <p>Key responsibilities include:
 *
 * <ul>
 *   <li>Consuming CDC (Change Data Capture) events for Organizations and Places.
...
public class OrganizationService {
            ...
            organizations.forEach(
                org -> {
                  ...
                  String reportingData =
                      transformer.processData(org, OrganizationType.ORGANIZATION_REPORTING);
                  kafkaTemplate.send(orgReportingOutputTopic, reportingKey, reportingData);
                  log.info(
                      "Organization data (uid={}) sent to {}", orgUid, orgReportingOutputTopic);
                  log.debug("Organization Reporting: {}", reportingData);

                  if (elasticSearchEnable) {
                    String elasticKey = transformer.buildOrganizationKey(org);
                    String elasticData =
                        transformer.processData(org, OrganizationType.ORGANIZATION_ELASTIC_SEARCH);
                    kafkaTemplate.send(orgElasticSearchTopic, elasticKey, elasticData);
                    log.info(
                        "Organization data (uid={}) sent to {}", orgUid, orgElasticSearchTopic);
  ...
  protected void processPhcFactDatamart(String uids) {
    try {
      ...
      log.info(
          "Stored proc execution completed: sp_public_health_case_fact_datamart_update '{}", uids);
    } catch (Exception dbe) {
        ...
        placeData
            ...
            .forEach(
                place -> {
                  ...
                  log.info(
                      "Place data (uid={}) sent to {}",
                      place.getPlaceUid(),
                      placeReportingOutputTopic);
                });
      } else {
    ...
    try {
      ...
      teleData.forEach(
          tele -> {
            ...
            log.info(
                "Place Tele data (uid={}) sent to {}",
                tele.getPlaceTeleLocatorUid(),
                teleOutputTopic);
          });

...
import static gov.cdc.etldatapipeline.postprocessing.service.Entity.*;
import static gov.cdc.etldatapipeline.postprocessing.service.ProcessDatamartData.MULTI_ID_DATAMART;
import static gov.cdc.etldatapipeline.postprocessing.service.ProcessDatamartData.STATUS_READY;
...
import gov.cdc.etldatapipeline.postprocessing.repository.InvestigationRepository;
import gov.cdc.etldatapipeline.postprocessing.repository.PostProcRepository;
import gov.cdc.etldatapipeline.postprocessing.repository.model.BackfillData;
import gov.cdc.etldatapipeline.postprocessing.repository.model.DatamartData;
import gov.cdc.etldatapipeline.postprocessing.repository.model.dto.Datamart;
...
public class PostProcessingService {
  ...
  // cache to store information from topics with key values that need to be type String as opposed
  // to Long
   ...
   *       tables.
   *   <li><b>Notification</b>: extract the act type code and link it to the associated
   *       investigation UID for summary case processing.
   *   <li><b>Observation</b>: classify observations into MorbReport or LabReport caches based on
   *       control and domain codes.
   * </ul>
   *
      ...
      if (topic.endsWith(INVESTIGATION.getEntityName())) {
        ...
        */
        ...
        String electronicInd =
            payloadNode.path("electronic_ind").asText(); // should always be there
        if (resUidNode != null
            && !Objects.equals(resUidNode.asText(), "")
  ...
  public void processDmMessage(
    ...
    try {
      ...
      if (Objects.isNull(dmData)) {
        logger.info(
            "For payload: {} DataMart object is null. Skipping further processing", payloadNode);
        return;
      }
      if (Objects.isNull(dmData.getPublicHealthCaseUid())
          || Objects.isNull(dmData.getPatientUid())) {
        logger.info(
            "For payload: {} DataMart Case/Patient Id is null. Skipping further processing",
            payloadNode);
        return;
      }
      if (Objects.isNull(dmData.getDatamart())) {
        logger.info(
            "For payload: {} DataMart value is null. Skipping further processing", payloadNode);
        return;
      }
  ...
  /**
   * Processes all cached IDs for various entities (e.g., investigations, notifications,
   * organizations) and executes the appropriate stored procedures for each entity type. This method
   * triggers at fixed intervals and consolidates the processing of IDs collected from multiple
   * Kafka messages.
   *
   * <p>Steps:
   *
   ...
   *
   * <ul>
   *   4. Store the entity name and ids for later if needed for data marts (both direct dependent
   *   and also for multi-id datamarts).
   * </ul>
   *
   * <ul>
   *   5. Process event metric and summary datamarts
   * </ul>
   *
   * <ul>
   *   6. Send the event message to the kafka topic that handles data mart events for building other
   *   datamarts
   * </ul>
   */
  ...
  protected void processCachedIds() {
    ...
    synchronized (cacheLock) {
      idCacheSnapshot =
          idCache.entrySet().stream()
              .collect(
                  Collectors.toMap(Map.Entry::getKey, entry -> new ArrayList<>(entry.getValue())));
      idCache.clear();

      cdCacheSnapshot =
          cdCache.entrySet().stream()
              .collect(
                  Collectors.toMap(Map.Entry::getKey, entry -> new ArrayList<>(entry.getValue())));
      cdCache.clear();

      pbCacheSnapshot =
          pbCache.entrySet().stream()
              .collect(
                  Collectors.toMap(Map.Entry::getKey, entry -> new ArrayList<>(entry.getValue())));
      pbCache.clear();

      obsCacheSnapshot =
          obsCache.entrySet().stream()
              .collect(
                  Collectors.toMap(Map.Entry::getKey, entry -> new ArrayList<>(entry.getValue())));
      obsCache.clear();
    }
  ...
  /**
   * Scheduled task to reprocess failed entities by iterating through retry cache and invoking
   * {@link #processIdCache(Map, Map, Map, Long)} with snapshots of the cached IDs.
   *
   * <ul>
    ...
    for (Entry<Long, Map<String, Queue<Long>>> retryEntry : retryCache.entrySet()) {
      ...
      if (processed) {
        ...
        if (attempt >= maxRetries) {
          ...
          dmProcessor.processBackfills(
              idCacheSnapshot, key -> getEntityByTopic(key).getEntityName().toUpperCase(), batchId);
    ...
    backfills.forEach(
        bf -> {
          ...
          if (bfe.startsWith("DM^")) {
            ...
            Queue<Long> queue =
                Arrays.stream(bf.getRecordUidList().split(","))
                    .map(Long::valueOf)
  ...
  private boolean processIdCache(
      Map<String, List<Long>> idCache,
      Map<String, List<Long>> pbCache,
      Map<String, List<Long>> obsCache,
      Long batchId) {

    // sorting idCacheSnapshot so that entities with higher priority is processed first
    List<Entry<String, List<Long>>> sortedEntries =
        idCache.entrySet().stream()
            .sorted(
                Comparator.comparingInt(entry -> getEntityByTopic(entry.getKey()).getPriority()))
            .toList();

    // list to store details of datamarts and the associated ids that needs to be hydrated
    // downstream
    ...
    for (Entry<String, List<String>> entry : cdCacheSnapshot.entrySet()) {
      ...
      try {
        if (Objects.requireNonNull(entity) == CONDITION) {
          processTopic(
              keyTopic, entity, cds, postProcRepository::executeStoredProcForConditionCode);
        } else {
        ...
        logger.error(
            errorMessage(entity.getEntityName(), ids, new Exception(e.getClass().getSimpleName())));
  ...
  private void logIdProcessing(int idSize, String keyTopic, Long batchId) {
    if (batchId == null) {
      ...
      logger.info(
          "Retrying {} id(s) from topic: {}, attempt #{} for batch id: {}",
          idSize,
          keyTopic,
          dmProcessor.retryAttempts.getOrDefault(batchId, 0) + 1,
          batchId);
    }
  }
  ...
  private List<DatamartData> processEntity(
      ...
      Map<String, List<Long>> pbCache,
      Map<String, List<Long>> obsCache,
      Map<String, Queue<Long>> newDmMulti) {
    ...
    switch (entity) {
      case PAGE:
        processTopic(
            ...
            entity,
            ids,
            postProcRepository::executeStoredProcForNBSPage,
            dmProcessor::checkResult);
        break;
      case ORGANIZATION:
        dmDataSp =
            processTopic(
                ...
                dmProcessor::checkResult);
        newDmMulti
            .computeIfAbsent(ORGANIZATION.getEntityName(), k -> new ConcurrentLinkedQueue<>())
            .addAll(ids);
        break;
      case PROVIDER:
        dmDataSp =
            processTopic(
                ...
                entity,
                ids,
                postProcRepository::executeStoredProcForProviderIds,
                dmProcessor::checkResult);
        newDmMulti
            .computeIfAbsent(PROVIDER.getEntityName(), k -> new ConcurrentLinkedQueue<>())
            .addAll(ids);
        break;
      case PATIENT:
        dmDataSp =
            processTopic(
                ...
                entity,
                ids,
                postProcRepository::executeStoredProcForPatientIds,
                dmProcessor::checkResult);
        newDmMulti
            .computeIfAbsent(PATIENT.getEntityName(), k -> new ConcurrentLinkedQueue<>())
            .addAll(ids);
      ...
      case AUTH_USER:
        processTopic(
            ...
            entity,
            ids,
            postProcRepository::executeStoredProcForUserProfile,
            dmProcessor::checkResult);
        break;
      case D_PLACE:
        processTopic(
            ...
            entity,
            ids,
            postProcRepository::executeStoredProcForDPlace,
            dmProcessor::checkResult);
        break;
      case INVESTIGATION:
      ...
      case CONTACT:
        dmDataSp =
            processTopic(
                ...
                entity,
                ids,
                postProcRepository::executeStoredProcForDContactRecord,
                dmProcessor::checkResult);
        processTopic(
            keyTopic,
            entity,
        ...
        newDmMulti
            .computeIfAbsent(CONTACT.getEntityName(), k -> new ConcurrentLinkedQueue<>())
            .addAll(ids);
        break;
      case NOTIFICATION:
        dmDataSp =
            processTopic(
                ...
                dmProcessor::checkResult);
        newDmMulti
            .computeIfAbsent(NOTIFICATION.getEntityName(), k -> new ConcurrentLinkedQueue<>())
            .addAll(ids);
        break;
      ...
      case STATE_DEFINED_FIELD_METADATA:
        processTopic(
            ...
            entity,
            ids,
            postProcRepository::executeStoredProcForLdfDimensionalData,
            dmProcessor::checkResult);
        break;
      case LDF_DATA:
        processTopic(
      ...
      case OBSERVATION:
        ...
        newDmMulti
            .computeIfAbsent(OBSERVATION.getEntityName(), k -> new ConcurrentLinkedQueue<>())
            .addAll(ids);
        break;
      case TREATMENT:
        processTopic(
            ...
            entity,
            ids,
            postProcRepository::executeStoredProcForTreatment,
            dmProcessor::checkResult);
        break;
      case VACCINATION:
        dmDataSp =
            processTopic(
                ...
                entity,
                ids,
                postProcRepository::executeStoredProcForDVaccination,
                dmProcessor::checkResult);
        processTopic(
            keyTopic,
            entity,
        ...
        newDmMulti
            .computeIfAbsent(VACCINATION.getEntityName(), k -> new ConcurrentLinkedQueue<>())
            .addAll(ids);
        break;
      default:
        logger.warn(UNKNOWN_TOPIC_LOG_MSG, keyTopic);
  ...
  private void processByInvFormCode(List<DatamartData> dmData, String keyTopic) {
    ...
    if (!pamUids.isEmpty()) {
      ...
      processTopic(
          ...
          D_MOVE_CNTRY,
          pamUids,
          investigationRepository::executeStoredProcForDMoveCntry,
          dmProcessor::checkResult);
      processTopic(
          keyTopic,
          D_MOVE_CNTY,
          pamUids,
          investigationRepository::executeStoredProcForDMoveCnty,
          dmProcessor::checkResult);
      processTopic(
          keyTopic,
          D_MOVE_STATE,
          pamUids,
          investigationRepository::executeStoredProcForDMoveState,
          dmProcessor::checkResult);
      processTopic(
          keyTopic,
          D_MOVED_WHERE,
          pamUids,
          investigationRepository::executeStoredProcForDMovedWhere,
          dmProcessor::checkResult);
      processTopic(
          keyTopic,
          D_HC_PROV_TY_3,
      ...
      processTopic(
          ...
          D_OUT_OF_CNTRY,
          pamUids,
          investigationRepository::executeStoredProcForDOutOfCntry,
          dmProcessor::checkResult);
      processTopic(
          keyTopic,
          D_SMR_EXAM_TY,
      ...
      processTopic(
          ...
          F_TB_PAM,
          pamUids,
          investigationRepository::executeStoredProcForFTbPam,
          dmProcessor::checkResult);
      processTopic(
          keyTopic,
          TB_PAM_LDF,
          pamUids,
          investigationRepository::executeStoredProcForTbPamLdf,
          dmProcessor::checkResult);
    }
    ...
    if (!pamUids.isEmpty()) {
      processTopic(
          ...
          D_VAR_PAM,
          pamUids,
          investigationRepository::executeStoredProcForDVarPam,
          dmProcessor::checkResult);
      processTopic(
          keyTopic,
          D_RASH_LOC_GEN,
          pamUids,
          investigationRepository::executeStoredProcForDRashLocGen,
          dmProcessor::checkResult);
      processTopic(
          keyTopic,
          D_PCR_SOURCE,
          pamUids,
          investigationRepository::executeStoredProcForDPcrSource,
          dmProcessor::checkResult);
      processTopic(
          keyTopic,
          F_VAR_PAM,
          pamUids,
          investigationRepository::executeStoredProcForFVarPam,
          dmProcessor::checkResult);
      processTopic(
          keyTopic,
          VAR_PAM_LDF,
          pamUids,
          investigationRepository::executeStoredProcForVarPamLdf,
          dmProcessor::checkResult);
    }
  }
  ...
  private List<DatamartData> processObservation(
    ...
    morbIds =
        Optional.ofNullable(obsCache.get(MORB_REPORT))
            .map(ArrayList::new)
            .orElseGet(ArrayList::new);
    labIds =
        Optional.ofNullable(obsCache.get(LAB_REPORT)).map(ArrayList::new).orElseGet(ArrayList::new);
    ...
    if (!morbIds.isEmpty()) {
      dmData =
          processTopic(
              ...
              entity,
              morbIds,
              postProcRepository::executeStoredProcForMorbReport,
              dmProcessor::checkResult,
    ...
    if (!labIds.isEmpty()) {
      processTopic(
          ...
          entity,
          labIds,
          postProcRepository::executeStoredProcForLabTest,
          dmProcessor::checkResult,
          "sp_d_lab_test_postprocessing");

      List<DatamartData> dmDataL =
          processTopic(
              keyTopic,
              entity,
              labIds,
              postProcRepository::executeStoredProcForLabTestResult,
              dmProcessor::checkResult,
      ...
      processTopic(
          ...
          entity,
          labIds,
          postProcRepository::executeStoredProcForLab100Datamart,
          dmProcessor::checkResult,
      ...
      processTopic(
          ...
          entity,
          labIds,
          postProcRepository::executeStoredProcForLab101Datamart,
          dmProcessor::checkResult,
  ...
  private void processSummaryCases() {
    ...
    synchronized (cacheLock) {
      sumUids =
          Optional.ofNullable(sumCache.get(CASE_TYPE_SUM))
              .map(HashSet::new)
              .orElse(new HashSet<>());
      aggUids =
          Optional.ofNullable(sumCache.get(CASE_TYPE_AGG))
              .map(HashSet::new)
              .orElse(new HashSet<>());
      sumCache.clear();
    }
  ...
  /**
   * Processes cached IDs for datamarts by executing the appropriate stored procedures. This method
   * triggers at fixed intervals and handles the processing of data that needs to be loaded into
   * specific datamarts.
   *
   * <p>Steps:
   *

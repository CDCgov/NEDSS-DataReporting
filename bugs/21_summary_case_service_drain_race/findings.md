...
- sp_summary_report_case_postprocessing's #tmp_SumRptWork CTE INNER JOINs nrt_investigation_observation
  ...
  drain, that obs row is not reliably materialized yet -> 0 rows. Re-running the SP after the drain
  completes produces the row immediately. => a drain-ordering race, not a data/routine bug.

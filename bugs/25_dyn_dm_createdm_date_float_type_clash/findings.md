...
that (under the intentional fail-fast) starves innocent co-batched entities -> more stable coverage.
...
1. Routine fix (in-bounds, sanctioned for a filed bug): make createdm type-aware. Before the UPDATE, for
   ...
   to also RECONCILE mismatched types. BLAST RADIUS: affects ALL dynamic datamarts createdm builds —

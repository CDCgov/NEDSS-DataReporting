-- ==========================================
-- USER: investigation_service_rdb
-- SERVICE: investigation-service
-- ==========================================
USE [master];

DECLARE @UserName NVARCHAR(150) ='investigation_service_rdb';
DECLARE @UserPassword NVARCHAR(128) = N'investigation_service'; --Please provide your generated password.
...
IF EXISTS (SELECT * FROM sys.database_principals WHERE name = @UserName)
    BEGIN
        ...
        -- Grant write permissions on SubjectRaceInfo
        DECLARE @GrantOdseWriteSubjectRaceSQL NVARCHAR(MAX) = 'GRANT INSERT, UPDATE, DELETE ON [dbo].[SubjectRaceInfo] TO [' + @UserName + ']';
        EXEC sp_executesql @GrantOdseWriteSubjectRaceSQL;
        PRINT 'Granted write permissions on [dbo].[SubjectRaceInfo] to [' + @UserName + ']';

        -- Grant write permissions on PublicHealthCaseFact
        DECLARE @GrantOdseWritePHCFactSQL NVARCHAR(MAX) = 'GRANT INSERT, UPDATE, DELETE ON [dbo].[PublicHealthCaseFact] TO [' + @UserName + ']';
...
IF EXISTS (SELECT * FROM sys.database_principals WHERE name = @UserName)
    BEGIN
        ...
        -- Grant execute sp_investigation_event
        DECLARE @GrantExecuteInvestigationEvent NVARCHAR(MAX) = 'GRANT EXECUTE ON [dbo].[sp_investigation_event] TO [' + @UserName + ']';
        EXEC sp_executesql @GrantExecuteInvestigationEvent;
        PRINT 'Granted EXECUTE permission on [dbo].[sp_investigation_event] to [' + @UserName + ']';
        
        -- Grant execute sp_public_health_case_fact_datamart_event
        DECLARE @GrantExecutePHCFactEvent NVARCHAR(MAX) = 'GRANT EXECUTE ON [dbo].[sp_public_health_case_fact_datamart_event] TO [' + @UserName + ']';

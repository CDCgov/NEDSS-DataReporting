-- ==========================================
-- USER: organization_service_rdb
-- SERVICE: organization-service
-- ==========================================
USE [master];

DECLARE @UserName NVARCHAR(150) = 'organization_service_rdb';
DECLARE @UserPassword NVARCHAR(128) = N'organization_service'; --Please provide your generated password.
...
IF EXISTS (SELECT * FROM sys.database_principals WHERE name = @UserName)
    BEGIN
        ...
        -- Grant update on PublicHealthCaseFact
        DECLARE @GrantOrgUpdatePHCFactSQL NVARCHAR(MAX) = 'GRANT UPDATE ON [dbo].[PublicHealthCaseFact] TO [' + @UserName + ']';
        EXEC sp_executesql @GrantOrgUpdatePHCFactSQL;
        PRINT 'Granted UPDATE permissions on [dbo].[PublicHealthCaseFact] to [' + @UserName + ']';
    END

...
IF EXISTS (SELECT * FROM sys.database_principals WHERE name = @UserName)
    BEGIN
        ...
        DECLARE @GrantExecuteLdfEvent NVARCHAR(MAX) = 'GRANT EXECUTE ON [dbo].[sp_ldf_data_event] TO [' + @UserName + ']';
        EXEC sp_executesql @GrantExecuteLdfEvent;

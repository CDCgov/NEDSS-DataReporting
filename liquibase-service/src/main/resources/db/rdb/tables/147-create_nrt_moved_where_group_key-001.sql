...
IF EXISTS (SELECT 1 FROM sysobjects WHERE name = 'nrt_moved_where_group_key' and xtype = 'U')
    BEGIN
        IF NOT EXISTS(SELECT 1 FROM sys.columns WHERE Name = N'created_dttm' AND Object_ID = Object_ID(N'nrt_moved_where_group_key'))
            BEGIN
                ALTER TABLE dbo.nrt_moved_where_group_key
                    ADD created_dttm DATETIME2 DEFAULT GETDATE();
            END;
        IF NOT EXISTS(SELECT 1 FROM sys.columns WHERE Name = N'updated_dttm' AND Object_ID = Object_ID(N'nrt_moved_where_group_key'))
            BEGIN
                ALTER TABLE dbo.nrt_moved_where_group_key
...
IF NOT EXISTS(SELECT 1 FROM sys.objects WHERE type = 'PK' AND parent_object_id = OBJECT_ID('dbo.nrt_moved_where_group_key'))
    BEGIN
        ALTER TABLE dbo.nrt_moved_where_group_key
        ADD CONSTRAINT pk_nrt_moved_where_group_key PRIMARY KEY (D_MOVED_WHERE_GROUP_KEY);
    END
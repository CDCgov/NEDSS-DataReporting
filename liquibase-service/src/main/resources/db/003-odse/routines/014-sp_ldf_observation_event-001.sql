IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_ldf_observation_event]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_ldf_observation_event]
END
GO 

CREATE PROCEDURE dbo.sp_ldf_observation_event @ldf_uid_list nvarchar(max), @bus_obj_uid_list nvarchar(max)
AS
Begin
    ...
    BEGIN TRY
        ...
        */
        ...
        select distinct m.ldf_uid,
                        ...
                        dbo.fn_get_record_status(o.record_status_cd) as record_status_cd,
        ...
        from  nbs_odse.dbo.State_Defined_Field_MetaData m
                  join nbs_odse.dbo.State_Defined_Field_Data d with (nolock) on m.ldf_uid = d.ldf_uid  and d.business_object_nm = 'LAB'
            ...
            and d.ldf_uid in (SELECT value FROM STRING_SPLIT(@ldf_uid_list, ','))
                  ...
                  join nbs_odse.dbo.Observation o with (nolock) on d.business_object_uid=o.observation_uid

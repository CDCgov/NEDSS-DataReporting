IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_crs_case_datamart_postprocessing]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_crs_case_datamart_postprocessing]
END
GO 

CREATE PROCEDURE dbo.sp_crs_case_datamart_postprocessing @phc_uids nvarchar(max),
                                                                  @debug bit = 'false'
as

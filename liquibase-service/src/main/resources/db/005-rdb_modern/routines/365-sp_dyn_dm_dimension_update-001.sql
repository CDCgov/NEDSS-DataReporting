...
BEGIN
    ...
    BEGIN TRY
        ...
        -- SET @Proc_Step_no = 1;
        -- SET @Proc_Step_Name = 'SP_Start - ' + @dimension_nm + ' Update for ' + @datamart_table;
        ...
        BEGIN
            SET @proc_step_name='UPDATE DYNAMIC DATAMART D_PATIENT VALUES';
            SET @proc_step_no = 6;
            ...
            BEGIN
                ...
                BEGIN
                        DECLARE @errmsg VARCHAR(2000) = 'Source table ''' +  @tbl_nm + ''' does not exist';
                ...
                -- IF COALESCE(@return_code, -1) = -1
                -- BEGIN
                --         set @error_flag = -1;
                --         set @datamart_error = @datamart_error + IIF(@err_ctr = 0, ' ', ', ') + @datamart_nm;
               ...
               -- reset return code for next iteration
               --SET @return_code = 0;

               FETCH NEXT FROM dyn_dm_data_cursor INTO @datamart_nm, @tbl_nm, @phc_uid_list;
          END;

        --   if COALESCE(@error_flag, -1) = -1
        --        BEGIN
        --             RAISERROR(@datamart_error, 16, 1)
        --        END

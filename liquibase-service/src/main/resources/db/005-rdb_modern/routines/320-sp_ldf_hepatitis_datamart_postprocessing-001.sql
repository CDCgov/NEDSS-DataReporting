IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_ldf_hepatitis_datamart_postprocessing]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_ldf_hepatitis_datamart_postprocessing]
END
GO 

CREATE PROCEDURE [dbo].[sp_ldf_hepatitis_datamart_postprocessing] 
...
@debug bit = 'false'
 as 
  BEGIN 
	...
	declare @dataflow_name varchar(200) = 'sp_ldf_hepatitis_datamart_postprocessing POST-Processing';
	declare @package_name varchar(200) = 'sp_ldf_hepatitis_datamart_postprocessing';
	...
	DECLARE @global_tmp_hepatitis_ta NVARCHAR(MAX);
	set @global_tmp_hepatitis_ta = '##TMP_HEPATITIS_TA_' + CAST(@batch_id as varchar(50));

	DECLARE @global_tmp_hepatitis_short_col NVARCHAR(MAX);
	set @global_tmp_hepatitis_short_col = '##TMP_HEPATITIS_SHORT_COL_' + CAST(@batch_id as varchar(50));

	DECLARE @global_tmp_hepatitis NVARCHAR(MAX);
	set @global_tmp_hepatitis = '##TMP_HEPATITIS_' + CAST(@batch_id as varchar(50));
		...
		( 
			...
			FROM dbo.LDF_DIMENSIONAL_DATA LDF_DIMENSIONAL_DATA  with (nolock) 
				INNER JOIN dbo.LDF_DATAMART_TABLE_REF   with (nolock) ON LDF_DIMENSIONAL_DATA.PHC_CD = dbo.LDF_DATAMART_TABLE_REF.condition_cd 
															AND DATAMART_NAME = upper('LDF_HEPATITIS')
...
--------------------------------------------------------------------------------------------------------				
			--- CREATE TABLE LINKED_HEPATITIS AS  
			SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
			...
			INTO  #TMP_LINKED_HEPATITIS 
			FROM 
				#TMP_BASE_HEPATITIS GEN_LDF  with (nolock) 
				INNER JOIN  dbo.INVESTIGATION INV with (nolock) 
			ON   
				GEN_LDF.INVESTIGATION_UID=INV.CASE_UID  
			INNER JOIN dbo.HEPATITIS_CASE GEN  with (nolock) 
			ON  
...
--------------------------------------------------------------------------------------------------------				
			...
			INTO #TMP_ALL_HEPATITIS 
			FROM 	dbo.LDF_DATAMART_COLUMN_REF  B with (nolock) 
			INNER JOIN #TMP_LINKED_HEPATITIS A with (nolock) 
	...
	--------------------------------------------------------------------------------------------------------				
			...
			INTO #TMP_ALL_HEPATITIS_SHORT_COL 
			FROM #TMP_ALL_HEPATITIS
	...
	--------------------------------------------------------------------------------------------------------		 
			...
			IF @count > 0 
			BEGIN 
				EXEC ('IF OBJECT_ID(''tempdb..' + @global_tmp_hepatitis_ta +''', ''U'')  IS NOT NULL
				BEGIN
					DROP TABLE ' + @global_tmp_hepatitis_ta +';
				END;')
	...
	--------------------------------------------------------------------------------------------------------
			...
			set @query =  
				'SELECT INVESTIGATION_KEY, 
						...
						DISEASE_CD 
					INTO '+ @global_tmp_hepatitis_ta + '
					FROM #TMP_ALL_HEPATITIS_TA with (nolock);';
	...
	--------------------------------------------------------------------------------------------------------
			...
			set @count = (SELECT count(*) FROM #TMP_ALL_HEPATITIS_SHORT_COL) 
			IF @count > 0 
			BEGIN 
			
				EXEC ('IF OBJECT_ID(''tempdb..' + @global_tmp_hepatitis_short_col +''', ''U'')  IS NOT NULL
					BEGIN
						DROP TABLE ' + @global_tmp_hepatitis_short_col +';
					END;')
				...
				SET @query=''; 
					...
					SELECT @cols = @cols + QUOTENAME(DATAMART_COLUMN_NM) + ',' FROM (select distinct DATAMART_COLUMN_NM from #TMP_ALL_HEPATITIS_SHORT_COL ) as tmp 
					select @cols = substring(@cols, 0, len(@cols)) --trim "," at end 
					
					set @query =  
					'SELECT * 
					INTO ' + @global_tmp_hepatitis_short_col +'
					FROM 
					(  
					SELECT     INVESTIGATION_KEY, 
							INVESTIGATION_LOCAL_ID, 
					...
					FROM #TMP_ALL_HEPATITIS_SHORT_COL ) 
					as A  
	...
	------------------------------------------------------------------------------------------------
			...
			VALUES (@batch_id, @dataflow_name, @package_name, 'START', @Proc_Step_no, @Proc_Step_Name,
					@RowCount_no);   
------------------------------------------------------------------------------------------------                            
			--- MERGE  HEPATITIS_SHORT_COL HEPATITIS_TA; 
					
			SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
			SET @PROC_STEP_NAME = ' GENERATING TMP_HEPATITIS';  

			EXEC ('IF OBJECT_ID(''tempdb..' + @global_tmp_hepatitis +''', ''U'')  IS NOT NULL
				BEGIN
					DROP TABLE ' + @global_tmp_hepatitis +';
				END;')


				EXECUTE  [dbo].[sp_MERGE_TABLES]  
					@INPUT_TABLE1= @global_tmp_hepatitis_short_col
					,@INPUT_TABLE2= @global_tmp_hepatitis_ta
					,@OUTPUT_TABLE= @global_tmp_hepatitis
					,@JOIN_ON_COLUMN='INVESTIGATION_KEY'
					,@batch_id = @batch_id
					,@target_table_name = 'LDF_HEPATITIS';

				set @query =  
					'DELETE FROM '+ @global_tmp_hepatitis +' WHERE INVESTIGATION_KEY IS NULL;'; 
					execute(@query) 
...
------------------------------------------------------------------------------------------------				
			...
			SET @PROC_STEP_NAME = 'GENERATING LDF_HEPATITIS';  
				...
				--- If the TMP_HEPATITIS has additional columns compare to LDF_HEPATITIS, add these additional columns in LDF_HEPATITIS table. 
				BEGIN TRANSACTION; 
					...
					SELECT   @Alterdynamiccolumnlist  = @Alterdynamiccolumnlist+ 'ALTER TABLE dbo.LDF_HEPATITIS ADD [' + name   +  '] varchar(4000) ', 
						@dynamiccolumnUpdate= @dynamiccolumnUpdate + 'LDF_HEPATITIS.[' +  name  + ']='  + ''+  @global_tmp_hepatitis +'.['  +name  + '] ,' 
					FROM  tempdb.Sys.Columns WHERE Object_ID = Object_ID('tempdb..'+ @global_tmp_hepatitis +'') 
...
------------------------------------------------------------------------------------------------
			...
			SET @PROC_STEP_NO = @PROC_STEP_NO + 1; 
			SET @PROC_STEP_NAME = 'Update LDF_HEPATITIS';  
			...
			IF @Alterdynamiccolumnlist IS NOT NULL AND @Alterdynamiccolumnlist!='' 
				...
				EXEC ('update  dbo.LDF_HEPATITIS  SET ' +   @dynamiccolumnUpdate + ' FROM '+@global_tmp_hepatitis +'      
				inner join  dbo.LDF_HEPATITIS  on  ' + @global_tmp_hepatitis +'.INVESTIGATION_LOCAL_ID =  dbo.LDF_HEPATITIS.INVESTIGATION_LOCAL_ID') 
...
------------------------------------------------------------------------------------------------
			...
			SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
			SET @PROC_STEP_NAME = 'Delete Existing to LDF_HEPATITIS';  
				--In case of updates, delete the existing ones and insert updated ones in LDF_HEPATITIS 
				EXEC ('DELETE FROM dbo.LDF_HEPATITIS WHERE INVESTIGATION_KEY IN (SELECT INVESTIGATION_KEY FROM ' + @global_tmp_hepatitis +' );');
...
------------------------------------------------------------------------------------------------                            
			...
			SET @PROC_STEP_NO = @PROC_STEP_NO + 1; 
			SET @PROC_STEP_NAME = 'Insert to LDF_HEPATITIS';  
				...
				--- During update if TMP_HEPATITIS has 4 columns updated only and the LDF_HEPATITIS has 7 columns then get column name dynamically from TMP_HEPATITIS and populate them. 
					...
					SELECT @dynamiccolumnList= @dynamiccolumnList +'['+ name +'],' FROM  tempdb.Sys.Columns WHERE Object_ID = Object_ID('tempdb..'+ @global_tmp_hepatitis) 
...
------------------------------------------------------------------------------------------------
			...
			SET @PROC_STEP_NO = @PROC_STEP_NO + 1; 
			SET @PROC_STEP_NAME = 'UPDATE LDF_HEPATITIS when there is no record in the LDF_DIMENSIONAL_DATA';  

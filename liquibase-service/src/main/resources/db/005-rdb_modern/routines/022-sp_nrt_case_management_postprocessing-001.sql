IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_nrt_case_management_postprocessing]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_nrt_case_management_postprocessing]
END
GO 

CREATE PROCEDURE dbo.sp_nrt_case_management_postprocessing @id_list nvarchar(max), @debug bit = 'false'
AS
BEGIN
    ...
    BEGIN TRY
        ...
        VALUES (@batch_id,@dataflow_name,@package_name,'COMPLETE',@proc_step_no,@proc_step_name,@rowcount,LEFT(@id_list, 500));
    ...
    DECLARE @FullErrorMessage VARCHAR(8000) =
        ...
        VALUES (@batch_id,current_timestamp,current_timestamp,@dataflow_name,@package_name,'ERROR',@Proc_Step_no,@proc_step_name,0,LEFT(@id_list, 500),@FullErrorMessage);

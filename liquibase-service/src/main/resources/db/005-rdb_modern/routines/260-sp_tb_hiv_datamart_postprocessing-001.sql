IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_tb_hiv_datamart_postprocessing]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_tb_hiv_datamart_postprocessing]
END
GO 

CREATE PROCEDURE [dbo].[sp_tb_hiv_datamart_postprocessing] 
...
--------------------------------------------------------------------------------------------------------
        ...
        INNER JOIN #S_INVESTIGATION_LIST S
            ON S.TB_PAM_UID = h.TB_PAM_UID
        INNER JOIN [dbo].TB_DATAMART d WITH(nolock) 
            ON S.INVESTIGATION_KEY = d.INVESTIGATION_KEY;
...
--------------------------------------------------------------------------------------------------------
        SET
            @PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
            @PROC_STEP_NAME = 'DELETE INCOMING RECORDS';
        ...
        BEGIN TRANSACTION
            ...
            SET
                @PROC_STEP_NO = @PROC_STEP_NO + 1;
            SET
                @PROC_STEP_NAME = 'INSERTING INCOMING RECORDS';

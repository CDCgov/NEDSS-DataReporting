...
--------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    IF NOT EXISTS (SELECT 1 FROM #VAC_LIST)
        BEGIN
            if @debug='true'
                PRINT 'No rows found in #VAC_LIST. Exiting procedure.';
            SELECT
                ...
                'Missing NRT Record: sp_covid_vaccination_datamart_postprocessing' AS stored_procedure,
...
--------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   with UID_CTE as (
	...
	SELECT DISTINCT
	        ...
	        (SELECT PATIENT_BIRTH_SEX FROM D_PATIENT WHERE PATIENT_UID = patient.PATIENT_MPR_UID) AS PATIENT_BIRTH_SEX,
	        patient.PATIENT_DOB,
	        ...
	        UPPER(patient.PATIENT_COUNTRY) AS PATIENT_COUNTRY,
	        patient.PATIENT_SSN,
	        patient.PATIENT_PRIMARY_OCCUPATION,
	        patient.PATIENT_MARITAL_STATUS,
	        REPLACE(patient.PATIENT_RACE_CALC_DETAILS, ' |', ';') AS PATIENT_RACE_CALC_DETAILS,
	        ...
	        ISNULL(provider.PROVIDER_STREET_ADDRESS_2, '') AS PROVIDER_STREET_ADDRESS_2,
	        provider.PROVIDER_CITY,
	        provider.PROVIDER_STATE_CODE,
	        provider.PROVIDER_ZIP,
	        provider.PROVIDER_COUNTY,
	        UPPER(provider.PROVIDER_COUNTRY) AS PROVIDER_COUNTRY,
	        org.ORGANIZATION_NAME,
	        org.ORGANIZATION_STREET_ADDRESS_1,
	        ISNULL(org.ORGANIZATION_STREET_ADDRESS_2, '') AS ORGANIZATION_STREET_ADDRESS_2,
	        org.ORGANIZATION_CITY,
	        org.ORGANIZATION_STATE_CODE,
	        org.ORGANIZATION_ZIP,
	        org.ORGANIZATION_COUNTY,
	        UPPER(org.ORGANIZATION_COUNTRY) AS ORGANIZATION_COUNTRY,

IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_dyn_dm_org_data_postprocessing]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_dyn_dm_org_data_postprocessing]
END
GO 

CREATE PROCEDURE dbo.sp_dyn_dm_org_data_postprocessing @batch_id BIGINT,
...
BEGIN
    BEGIN TRY
        ...
        DECLARE @tmp_DynDm_ORGANIZATION varchar(200) = 'dbo.tmp_DynDm_Organization_' + @DATAMART_NAME + '_' +
            ...
            BEGIN
                ...
                SET @SQL = ' insert into #tmp_DynDm_OrgPart_Table_temp '
                    ...
                    + ' SELECT  distinct ' +
                           ...
                           ' INNER JOIN dbo.' + @FACT_CASE + '   ON s_d.INVESTIGATION_KEY =  ' + @FACT_CASE +
                           '.INVESTIGATION_KEY ' +
                           ' LEFT JOIN dbo.D_ORGANIZATION  d_o ON ' + @FACT_CASE + '.' + @PART_TYPE_CD +
                           ' = d_o.ORGANIZATION_KEY  ' +
                           '; '
                ...
                VALUES (@batch_id, @Dataflow_Name, @Package_Name, 'START', @Proc_Step_no,
                        @Proc_Step_Name + '-' + @PART_TYPE_CD, @ROWCOUNT_NO);
                ...
                update #tmp_DynDm_OrgPart_Table_temp
                SET DETAIL ='<b></b>' + LTRIM(RTRIM(coalesce(ORGANIZATION_LOCAL_ID, '')))
                where LTRIM(RTRIM(ORGANIZATION_LOCAL_ID)) is not null;
                update #tmp_DynDm_OrgPart_Table_temp
                SET DETAIL = DETAIL + '<br>' + (ORGANIZATION_NAME)
                where LTRIM(RTRIM(ORGANIZATION_NAME)) is not null;
                update #tmp_DynDm_OrgPart_Table_temp
                SET DETAIL= DETAIL + '<br>' + LTRIM(RTRIM(ORGANIZATION_STREET_ADDRESS_1))
                where LTRIM(RTRIM(ORGANIZATION_STREET_ADDRESS_1)) is not null;
                update #tmp_DynDm_OrgPart_Table_temp
                SET DETAIL= DETAIL + '<br>' + LTRIM(RTRIM(ORGANIZATION_STREET_ADDRESS_2))
                where LTRIM(RTRIM(ORGANIZATION_STREET_ADDRESS_2)) is not null;
                update #tmp_DynDm_OrgPart_Table_temp
                SET DETAIL= DETAIL + '<br>' + LTRIM(RTRIM(CITY_STATE_ZIP))
                where LTRIM(RTRIM(CITY_STATE_ZIP)) is not null;
                update #tmp_DynDm_OrgPart_Table_temp
                SET DETAIL= DETAIL + '<br>' + LTRIM(RTRIM(ORGANIZATION_COUNTY))
                where LTRIM(RTRIM(ORGANIZATION_COUNTY)) is not null;
                update #tmp_DynDm_OrgPart_Table_temp
                SET DETAIL= DETAIL + '<br>' + LTRIM(RTRIM(ORGANIZATION_PHONE_WORK))
                where LTRIM(RTRIM(ORGANIZATION_PHONE_WORK)) is not null;
                update #tmp_DynDm_OrgPart_Table_temp
                SET DETAIL= DETAIL + ', ext. ' + LTRIM(RTRIM(ORGANIZATION_PHONE_EXT_WORK))
                where LTRIM(RTRIM(ORGANIZATION_PHONE_WORK)) is not null
                  and LTRIM(RTRIM(ORGANIZATION_PHONE_EXT_WORK)) is not null;
                update #tmp_DynDm_OrgPart_Table_temp
                SET DETAIL= DETAIL + '<br> ext. ' + LTRIM(RTRIM(ORGANIZATION_PHONE_EXT_WORK))
                where LTRIM(RTRIM(ORGANIZATION_PHONE_WORK)) is null
                  and LTRIM(RTRIM(ORGANIZATION_PHONE_EXT_WORK)) is not null;
                update #tmp_DynDm_OrgPart_Table_temp
                SET DETAIL= DETAIL + '<br> '
                ...
                SET @Proc_Step_no = @Proc_Step_no + 1;
                SET @Proc_Step_Name =
                        ' Executing ALTER on #tmp_DynDm_OrgPart_Table_temp and ' + @tmp_DynDm_ORGANIZATION;


                SET @SQL = 'alter table #tmp_DynDm_OrgPart_Table_temp add  ' + @DETAIL + ' [varchar](2000) , ' +
                           @USER_DEFINED_COLUMN_NM + ' bigint , ' + @QEC + ' [varchar](50) , ' + @UID + ' bigint ; '

                EXEC (@SQL);

                SET @SQL = 'alter table ' + @tmp_DynDm_ORGANIZATION + ' add  ' + @DETAIL + ' [varchar](2000) , ' +
                           @USER_DEFINED_COLUMN_NM + ' bigint , ' + @QEC + ' [varchar](50) , ' + @UID + ' bigint ; '

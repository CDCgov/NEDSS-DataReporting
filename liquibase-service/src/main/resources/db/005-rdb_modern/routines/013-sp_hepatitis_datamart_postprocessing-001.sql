IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_hepatitis_datamart_postprocessing]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_hepatitis_datamart_postprocessing]
END
GO 
...
-------------------------------------------------------23. CREATE TABLE TMP_VAC_REPEAT_OUT_FINAL1--------------------------------------------------------------------------
        ...
        FROM dbo.[HEPATITIS_DATAMART] ISD
                 INNER OUTER JOIN #TMP_Event E WITH (NOLOCK) ON E.INVESTIGATION_KEY=ISD.INVESTIGATION_KEY;

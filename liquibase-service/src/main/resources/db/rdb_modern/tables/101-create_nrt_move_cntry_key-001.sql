...
IF EXISTS (SELECT 1 FROM sysobjects WHERE name = 'nrt_move_cntry_key' and xtype = 'U')
    BEGIN
        IF NOT EXISTS(SELECT 1 FROM sys.columns WHERE Name = N'created_dttm' AND Object_ID = Object_ID(N'nrt_move_cntry_key'))
            BEGIN
                ALTER TABLE dbo.nrt_move_cntry_key
                    ADD created_dttm DATETIME2 DEFAULT GETDATE();
            END;
        IF NOT EXISTS(SELECT 1 FROM sys.columns WHERE Name = N'updated_dttm' AND Object_ID = Object_ID(N'nrt_move_cntry_key'))
            BEGIN
                ALTER TABLE dbo.nrt_move_cntry_key
...
IF NOT EXISTS(SELECT 1 FROM sys.objects WHERE type = 'PK' AND parent_object_id = OBJECT_ID('dbo.nrt_move_cntry_key'))
    BEGIN
        ALTER TABLE dbo.nrt_move_cntry_key
        ADD CONSTRAINT pk_nrt_move_cntry_key PRIMARY KEY (D_MOVE_CNTRY_KEY);
    END

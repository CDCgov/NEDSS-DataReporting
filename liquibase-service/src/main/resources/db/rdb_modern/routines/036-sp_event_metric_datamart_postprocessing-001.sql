CREATE OR ALTER PROCEDURE dbo.sp_event_metric_datamart_postprocessing
...
BEGIN
    ...
    VALUES
        ...
        */
		...
		SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
									...
									IF OBJECT_ID('#TMP_EVENT_METRIC', 'U') IS NOT NULL   
    ...
    IF @notif_uids != ''
        ...
        IF OBJECT_ID('#TMP_NOTIFICATION', 'U') IS NOT NULL   
        ...
        VALUES
            (@batch_id, @datamart_nm, @datamart_nm, 'START', @Proc_Step_no, @Proc_Step_Name,
                @RowCount_no);
        ...
        IF OBJECT_ID('#TMP_NOT_PROG', 'U') IS NOT NULL   
        ...
        IF OBJECT_ID('#TMP_NOT_PROG_JURI', 'U') IS NOT NULL   
        ...
        IF OBJECT_ID('#TMP_NOT_PROG_JURI_CVG', 'U') IS NOT NULL   
        ...
        SET @Proc_Step_name='Inserting Notifications into #TMP_EVENT_METRIC';
        SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
    ...
    BEGIN
        ...
        SET @Proc_Step_name='Generating #TMP_EVENT_OBS';
        SET @PROC_STEP_NO = @PROC_STEP_NO + 1;


        IF OBJECT_ID('#TMP_EVENT_OBS', 'U') IS NOT NULL   
        ...
        SELECT
            ...
            o.local_id,
            pat.local_id as Local_Patient_ID ,
            CASE WHEN o.ctrl_cd_display_form = 'MorbReport' THEN o.cd
            ELSE NULL
        END as Condition_cd,
            CASE WHEN o.ctrl_cd_display_form = 'MorbReport' THEN q.condition_short_nm
            ELSE NULL
        ...
        FROM dbo.nrt_observation o
            ...
            LEFT JOIN dbo.nrt_patient pat                     with (nolock) ON pat.patient_uid  = o.patient_id
        ...
        SET @Proc_Step_name='Inserting Non-ELR LabReports into #TMP_EVENT_METRIC';
        SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
        ...
        SET @Proc_Step_name='Inserting ELR LabReports into #TMP_EVENT_METRIC';
        SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
        ...
        SET @Proc_Step_name='Inserting Morbidity Reports into #TMP_EVENT_METRIC';
        SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
    ...
    BEGIN
        ...
        SET @Proc_Step_name='Inserting Public Health Cases into #TMP_EVENT_METRIC';
        SET @PROC_STEP_NO = @PROC_STEP_NO + 1;


        INSERT INTO #TMP_EVENT_METRIC
        SELECT
            'PHCInvForm' as Event_Type,
    ...
    BEGIN
        ...
        SET @Proc_Step_name='Inserting CT_Contact Records into #TMP_EVENT_METRIC';
        SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
        ...
        FROM dbo.nrt_contact ct
            INNER JOIN [NBS_ODSE].dbo.PERSON                   with (nolock) ON PERSON.PERSON_UID = ct.SUBJECT_ENTITY_UID
            INNER JOIN [NBS_SRTE].dbo.PROGRAM_AREA_CODE AS P   with (nolock) ON ct.PROG_AREA_CD = P.PROG_AREA_CD
            INNER JOIN [NBS_SRTE].dbo.JURISDICTION_CODE AS J   with (nolock) ON ct.JURISDICTION_CD = J.CODE
            INNER JOIN [NBS_SRTE].dbo.CODE_VALUE_GENERAL C     with (nolock) ON ct.RECORD_STATUS_CD = C.CODE AND C.CODE_SET_NM='REC_STAT'
            LEFT OUTER JOIN dbo.nrt_auth_user AS UP1 with (nolock) ON ct.ADD_USER_ID = UP1.NEDSS_ENTRY_ID
            LEFT OUTER JOIN dbo.nrt_auth_user AS UP2 with (nolock) ON ct.LAST_CHG_USER_ID = UP2.NEDSS_ENTRY_ID
    ...
    BEGIN TRANSACTION
							...
							SET	[EVENT_TYPE]=TEM.[EVENT_TYPE],
							   ...
							   WHERE TEM.[EVENT_UID] = [dbo].[EVENT_METRIC_INC].[EVENT_UID]
    ...
    VALUES
        ...
        BEGIN TRANSACTION
					           ...
					           SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
					           SET @PROC_STEP_NAME = 'Update [dbo].[EVENT_METRIC]'; 
							...
							SET	[EVENT_TYPE]=TEM.[EVENT_TYPE],
								...
								[LAST_CHG_USER_NAME]=TEM.[LAST_CHG_USER_NAME]
								FROM #TMP_EVENT_METRIC TEM
							   WHERE TEM.[EVENT_UID] = [dbo].[EVENT_METRIC].[EVENT_UID]
    ...
    VALUES
        ...
        BEGIN TRANSACTION
                               SET @PROC_STEP_NO = @PROC_STEP_NO + 1;
					           SET @PROC_STEP_NAME = 'INSERTING INTO EVENT_METRIC'; 					

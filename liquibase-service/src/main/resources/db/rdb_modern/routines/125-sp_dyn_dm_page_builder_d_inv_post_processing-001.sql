...
CREATE or replace PROCEDURE [dbo].sp_dyn_dm_page_builder_d_inv_post_processing
		...
		if  object_id('dbo.'+@RDB_TABLE_NM) is not null
		  Begin
			 ...
			 SET @SQL = '   SELECT distinct  '+@listStr + ' tmp_DynDM_SUMM_DATAMART.INVESTIGATION_KEY ' +
						...
						'	  WHERE tmp_DynDm_SUMM_DATAMART.DISEASE_GRP_CD = (SELECT FORM_CD FROM dbo.NBS_PAGE WHERE DATAMART_NM='''+@DATAMART_NAME+''' )'
						;

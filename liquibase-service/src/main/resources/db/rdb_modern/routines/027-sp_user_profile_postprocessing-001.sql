IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_user_profile_postprocessing]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_user_profile_postprocessing]
END
GO 

CREATE PROCEDURE [dbo].[sp_user_profile_postprocessing] @id_list nvarchar(max), @debug bit = 'false'
AS
BEGIN

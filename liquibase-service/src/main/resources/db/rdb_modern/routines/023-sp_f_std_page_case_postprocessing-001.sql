...
BEGIN TRY
   ...
   SET @batch_id = cast((format(getdate(),'yyMMddHHmmss')) as bigint);
    ...
    FROM #PHC_CASE_UIDS_ALL   FSSHC
             LEFT OUTER JOIN dbo.CONDITION CONDITION ON FSSHC.CD= CONDITION.CONDITION_CD
             LEFT OUTER JOIN dbo.D_PATIENT PATIENT	 ON fsshc.PATIENT_ID= PATIENT.PATIENT_UID
             LEFT OUTER JOIN dbo.D_ORGANIZATION  HOSPITAL ON fsshc.HOSPITAL_UID= HOSPITAL.ORGANIZATION_UID
             LEFT OUTER JOIN dbo.D_ORGANIZATION  HOSPDELIVERY ON fsshc.ORG_AS_HOSPITAL_OF_DELIVERY_UID= HOSPDELIVERY.ORGANIZATION_UID
             LEFT OUTER JOIN dbo.D_ORGANIZATION REPORTERORG ON fsshc.ORG_AS_REPORTER_UID= REPORTERORG.ORGANIZATION_UID
             LEFT OUTER JOIN dbo.D_ORGANIZATION FACILITYORG ON fsshc.ORDERING_FACILITY_UID= FACILITYORG.ORGANIZATION_UID
             LEFT OUTER JOIN dbo.D_PROVIDER PERSONREPORTER ON  fsshc.PERSON_AS_REPORTER_UID= PERSONREPORTER.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER PROVDELIVERY ON fsshc.PER_AS_PROVIDER_OF_DELIVERY_UID= PROVDELIVERY.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER MOTHEROBGYN ON fsshc.PER_AS_PROVIDER_OF_OBGYN_UID= MOTHEROBGYN.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER PEDIATRICIAN ON fsshc.PER_AS_PROVIDER_OF_PEDIATRICS_UID= PEDIATRICIAN.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER PROVIDER ON fsshc.INVESTIGATOR_ID= PROVIDER.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER PHYSICIAN ON fsshc.PHYSICIAN_ID= PHYSICIAN.PROVIDER_UID
             LEFT OUTER JOIN dbo.INVESTIGATION  INVESTIGATION ON fsshc.PAGE_CASE_UID= INVESTIGATION.CASE_UID
             LEFT OUTER JOIN dbo.D_PROVIDER CL ON fsshc.CLOSURE_INVESTGR_OF_PHC_UID= CL.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER DISP ON fsshc.DISPO_FLD_FUPINVESTGR_OF_PHC_UID= DISP.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_ORGANIZATION  FACILITY ON fsshc.FLD_FUP_FACILITY_OF_PHC_UID= FACILITY.ORGANIZATION_UID
             LEFT OUTER JOIN dbo.D_PROVIDER FLD_FUP_INVESTGTR ON fsshc.FLD_FUP_INVESTGR_OF_PHC_UID= FLD_FUP_INVESTGTR.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER PROVIDER_FLD_FUP ON fsshc.FLD_FUP_PROV_OF_PHC_UID= PROVIDER_FLD_FUP.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER SUPRVSR_FLD_FUP ON fsshc.FLD_FUP_SUPERVISOR_OF_PHC_UID= SUPRVSR_FLD_FUP.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER INIT_FLD_FUP ON fsshc.INIT_FLD_FUP_INVESTGR_OF_PHC_UID= INIT_FLD_FUP.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER INIT_INVSTGR_PHC ON fsshc.INIT_FUP_INVESTGR_OF_PHC_UID= INIT_INVSTGR_PHC.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER INIT_INTERVIEWER ON fsshc.INIT_INTERVIEWER_OF_PHC_UID= INIT_INTERVIEWER.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER INTERVIEWER ON fsshc.INTERVIEWER_OF_PHC_UID= INTERVIEWER.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER SURV ON fsshc.SURV_INVESTGR_OF_PHC_UID= SURV.PROVIDER_UID
             LEFT OUTER JOIN dbo.D_PROVIDER CA ON fsshc.CA_SUPERVISOROFPHC_UID= CA.PROVIDER_UID
    ...
    from  dbo.L_INV_ADMINISTRATIVE union
    select PAGE_CASE_UID 	 from  dbo.L_INV_CLINICAL union
    select PAGE_CASE_UID 	 from  dbo.L_INV_COMPLICATION union
    select PAGE_CASE_UID 	 from  dbo.L_INV_CONTACT union
    select PAGE_CASE_UID 	 from  dbo.L_INV_DEATH union
    select PAGE_CASE_UID 	 from  dbo.L_INV_EPIDEMIOLOGY union
    select PAGE_CASE_UID 	 from  dbo.L_INV_HIV union
    select PAGE_CASE_UID 	 from  dbo.L_INV_ISOLATE_TRACKING union
    select PAGE_CASE_UID 	 from  dbo.L_INV_LAB_FINDING union
    select PAGE_CASE_UID 	 from  dbo.L_INV_MEDICAL_HISTORY union
    select PAGE_CASE_UID 	 from  dbo.L_INV_MOTHER union
    select PAGE_CASE_UID 	 from  dbo.L_INV_OTHER union
    select PAGE_CASE_UID 	 from  dbo.L_INV_PATIENT_OBS union
    select PAGE_CASE_UID 	 from  dbo.L_INV_PREGNANCY_BIRTH union
    select PAGE_CASE_UID 	 from  dbo.L_INV_RESIDENCY union
    select PAGE_CASE_UID 	 from  dbo.L_INV_RISK_FACTOR union
    select PAGE_CASE_UID 	 from  dbo.L_INV_SOCIAL_HISTORY union
    select PAGE_CASE_UID 	 from  dbo.L_INV_SYMPTOM union
    select PAGE_CASE_UID 	 from  dbo.L_INV_TRAVEL union
    select PAGE_CASE_UID 	 from  dbo.L_INV_TREATMENT union
    select PAGE_CASE_UID 	 from  dbo.L_INV_UNDER_CONDITION union
    select PAGE_CASE_UID 	 from  dbo.L_INV_VACCINATION union
    SELECT PAGE_CASE_UID	    from  dbo.L_INVESTIGATION_REPEAT union
    SELECT PAGE_CASE_UID	    from  dbo.L_INV_PLACE_REPEAT
    ;
    ...
    from #DIMENSION_KEYS_PAGECASEID DIMC
             LEFT OUTER JOIN   dbo.L_INV_ADMINISTRATIVE ladmin ON  ladmin.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_CLINICAL licl ON  licl.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_COMPLICATION licomp ON  licomp.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_CONTACT licon ON  licon.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_DEATH lid ON  lid.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_EPIDEMIOLOGY lie ON  lie.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_HIV lihiv ON  lihiv.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_ISOLATE_TRACKING liit ON  liit.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_LAB_FINDING lilf ON  lilf.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_MEDICAL_HISTORY limh ON  limh.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_MOTHER lim ON  lim.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_OTHER liot ON  liot.PAGE_CASE_UID = dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_PATIENT_OBS lipo ON  lipo.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_PREGNANCY_BIRTH lipb ON  lipb.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_RESIDENCY lirs ON  lirs.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_RISK_FACTOR lirf ON  lirf.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_SOCIAL_HISTORY lish ON  lish.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_SYMPTOM lis ON  lis.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_TRAVEL litl ON  litl.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_TREATMENT litr ON  litr.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_UNDER_CONDITION liuc ON liuc.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_VACCINATION liv ON  liv.PAGE_CASE_UID  =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INVESTIGATION_REPEAT lir ON lir.PAGE_CASE_UID =  dimc.page_case_uid
             LEFT OUTER JOIN   dbo.L_INV_PLACE_REPEAT lipr ON  lipr.PAGE_CASE_UID =  dimc.page_case_uid
    ;
    ...
    FROM #DIMENSIONAL_KEYS as DIM_KEYS
             ...
             LEFT OUTER JOIN dbo.RDB_DATE DATE1
                             ON cast(DATE1.DATE_MM_DD_YYYY as date) = cast(KEYSTORE.ADD_TIME as date)
             LEFT OUTER JOIN dbo.RDB_DATE DATE2 ON cast(DATE2.DATE_MM_DD_YYYY as date) =
                                                   cast(KEYSTORE.LAST_CHG_TIME as date)
    ;
    ...
    FROM #F_STD_PAGE_CASE_TEMP_INC pti
             inner join dbo.INVESTIGATION inv on inv.INVESTIGATION_KEY = pti.INVESTIGATION_KEY
             inner join dbo.D_PATIENT pt on pt.PATIENT_KEY = pti.PATIENT_KEY
             inner join dbo.CONDITION cd on cd.CONDITION_KEY = pti.CONDITION_KEY

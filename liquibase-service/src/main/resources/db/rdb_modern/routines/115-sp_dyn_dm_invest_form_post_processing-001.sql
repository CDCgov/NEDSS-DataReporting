...
--CREATE TABLE INV_SUMM_DATAMART AS
	...
	INNER JOIN #tmp_DynDm_Investigation_Data ON tmp_DynDm_Investigation_Data.INVESTIGATION_KEY = INV_SUMM_DATAMART.INVESTIGATION_KEY
	inner join dbo.nrt_investigation_key nrt_inv on nrt_inv.INVESTIGATION_KEY =  tmp_DynDm_Investigation_Data.INVESTIGATION_KEY
	and  nrt_inv_key.case_uid in (SELECT value FROM STRING_SPLIT(@phc_id_list, ','));

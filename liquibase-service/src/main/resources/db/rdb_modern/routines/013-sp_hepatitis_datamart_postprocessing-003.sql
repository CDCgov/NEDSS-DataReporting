...
BEGIN
    ...
    BEGIN TRY
        ...
        VALUES( @batch_id, 'HEPATITIS_DATAMART', 'RDB.Hepatitis_Case_DATAMART', 'START', @Proc_Step_no, @Proc_Step_Name, 0 );
        ...
        VALUES( @BATCH_ID, 'HEPATITIS_DATAMART', 'RDB.Hepatitis_Case_DATAMART', 'START', @PROC_STEP_NO, @PROC_STEP_NAME, @ROWCOUNT_NO );
...
*/
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'CLN_HepDInfection'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_CLINICAL'))
            BEGIN
                ...
                SET @sql = N'update rdb.dbo.TMP_D_INV_CLINICAL set HEP_D_INFECTION_IND=RTRIM(LTRIM(CLN_HepDInfection));'
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'CLN_MedsforHep'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_CLINICAL'))
            BEGIN
                --declare @sql nvarchar(100)
                SET @sql = N'update rdb.dbo.TMP_D_INV_CLINICAL set HEP_MEDS_RECVD_IND=RTRIM(LTRIM(CLN_MedsforHep));'
                execute sp_executesql  @sql
            END;
        ...
        FROM rdb.dbo.TMP_F_INV_PATIENT_OBS AS F
                 LEFT JOIN	 rdb.[dbo].[D_INV_PATIENT_OBS] AS D WITH(NOLOCK)  ON F.D_INV_PATIENT_OBS_KEY = D.D_INV_PATIENT_OBS_KEY
        ORDER BY F.INVESTIGATION_KEY;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'IPO_SEXUAL_PREF'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_PATIENT_OBS'))
            BEGIN
                --declare @sql nvarchar(100)
                SET @sql = N'update rdb.dbo.TMP_D_INV_PATIENT_OBS set SEX_PREF=RTRIM(LTRIM(IPO_SEXUAL_PREF));'
                execute sp_executesql  @sql
            END;
...
*/
        ...
        FROM rdb.dbo.TMP_F_INV_EPIDEMIOLOGY AS F
                 LEFT JOIN rdb.[dbo].[D_INV_EPIDEMIOLOGY] AS D WITH(NOLOCK)  ON F.D_INV_EPIDEMIOLOGY_KEY = D.D_INV_EPIDEMIOLOGY_KEY
        ORDER BY F.INVESTIGATION_KEY;
...
--				declare @sql nvarchar(100)
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_OutbreakAssoc'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set COM_SRC_OUTBREAK_IND=RTRIM(LTRIM(EPI_OutbreakAssoc));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_ChildCareCase'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set CHILDCARE_CASE_IND = RTRIM(LTRIM(EPI_ChildCareCase));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_CNTRY_USUAL_RESID'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set CNTRY_USUAL_RESIDENCE = RTRIM(LTRIM(EPI_CNTRY_USUAL_RESID));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_ContactBabysitter'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set CT_BABYSITTER_IND = RTRIM(LTRIM(EPI_ContactBabysitter));'
                execute sp_executesql  @sql
            END;


        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_ContactChildcare'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set CT_CHILDCARE_IND = RTRIM(LTRIM(EPI_ContactChildcare));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_ContactHousehold'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set CT_HOUSEHOLD_IND = RTRIM(LTRIM(EPI_ContactHousehold));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_ContactOfCase'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set HEP_CONTACT_IND = RTRIM(LTRIM(EPI_ContactOfCase));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_ContactOther'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set OTHER_CONTACT_IND = RTRIM(LTRIM(EPI_ContactOther));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_ContactOthSpecify'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set CONTACT_TYPE_OTH = RTRIM(LTRIM(EPI_ContactOthSpecify));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_ContactPlaymate'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set CT_PLAYMATE_IND = RTRIM(LTRIM(EPI_ContactPlaymate));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_ContactSexPartner'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set SEXUAL_PARTNER_IND = RTRIM(LTRIM(EPI_ContactSexPartner));'
                execute sp_executesql  @sql
            END;


        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_DaycareContact'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set DNP_HOUSEHOLD_CT_IND = RTRIM(LTRIM(EPI_DaycareContact));'
                execute sp_executesql  @sql
            END;

        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_EpiLinked'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set HEP_A_EPLINK_IND = RTRIM(LTRIM(EPI_EpiLinked));'
                execute sp_executesql  @sql
            END;

        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_FemaleSexPartners'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set FEMALE_SEX_PRTNR_NBR = (EPI_FemaleSexPartners);'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_FoodHandler'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set FOODHNDLR_PRIOR_IND = RTRIM(LTRIM(EPI_FoodHandler));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_InDayCare'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set DNP_EMPLOYEE_IND = RTRIM(LTRIM(EPI_InDayCare));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_IVDrugUse'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set STREET_DRUG_INJECTED = RTRIM(LTRIM(EPI_IVDrugUse));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_MaleSexPartner'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set MALE_SEX_PRTNR_NBR = RTRIM(LTRIM(EPI_MaleSexPartner));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_OutbreakFoodHndlr'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set OBRK_FOODHNDLR_IND = RTRIM(LTRIM(EPI_OutbreakFoodHndlr));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_OutbreakFoodItem'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set FOOD_OBRK_FOOD_ITEM = RTRIM(LTRIM(EPI_OutbreakFoodItem));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_outbreakNonFoodHndlr'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set OBRK_NOFOODHNDLR_IND = RTRIM(LTRIM(EPI_outbreakNonFoodHndlr));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_OutbreakUnidentified'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set OBRK_UNIDENTIFIED_IND = RTRIM(LTRIM(EPI_OutbreakUnidentified));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_OutbreakWaterborne'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set OBRK_WATERBORNE_IND = RTRIM(LTRIM(EPI_OutbreakWaterborne));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'EPI_RecDrugUse'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_EPIDEMIOLOGY'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_EPIDEMIOLOGY set STREET_DRUG_USED = RTRIM(LTRIM(EPI_RecDrugUse));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_TotalAntiHCV'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HEP_C_TOTAL_ANTIBODY = RTRIM(LTRIM(LAB_TotalAntiHCV));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_ALT_Result'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set ALT_SGPT_RESULT = RTRIM(LTRIM(LAB_ALT_Result));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_AntiHBsPositive'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set ANTI_HBS_POS_REAC_IND = RTRIM(LTRIM(LAB_AntiHBsPositive));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_AntiHBsTested'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set ANTI_HBSAG_TESTED_IND = RTRIM(LTRIM(LAB_AntiHBsTested));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_AST_Result'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set AST_SGOT_RESULT = RTRIM(LTRIM(LAB_AST_Result));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_HBeAg'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HEP_E_ANTIGEN = RTRIM(LTRIM(LAB_HBeAg));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_HBsAg'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HEP_B_SURFACE_ANTIGEN = RTRIM(LTRIM(LAB_HBsAg));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_HBV_NAT'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HEP_B_DNA = RTRIM(LTRIM(LAB_HBV_NAT));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_HCVRNA'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HCV_RNA = RTRIM(LTRIM(LAB_HCVRNA));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_HepDTest'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HEP_D_TEST_IND = RTRIM(LTRIM(LAB_HepDTest));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_IgM_AntiHAV'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HEP_A_IGM_ANTIBODY = RTRIM(LTRIM(LAB_IgM_AntiHAV));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_IgMAntiHBc'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HEP_B_IGM_ANTIBODY = RTRIM(LTRIM(LAB_IgMAntiHBc));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_PrevNegHepTest'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set PREV_NEG_HEP_TEST_IND = RTRIM(LTRIM(LAB_PrevNegHepTest));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_SignalToCutoff'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set ANTIHCV_SIGCUT_RATIO = RTRIM(LTRIM(LAB_SignalToCutoff));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_Supplem_antiHCV'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set ANTIHCV_SUPP_ASSAY = RTRIM(LTRIM(LAB_Supplem_antiHCV));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_TestResultUpperLimit'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set ALT_SGPT_RSLT_UP_LMT = RTRIM(LTRIM(LAB_TestResultUpperLimit));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_TestResultUpperLimit2'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set AST_SGOT_RSLT_UP_LMT = RTRIM(LTRIM(LAB_TestResultUpperLimit2));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_TotalAntiHAV'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HEP_A_TOTAL_ANTIBODY = RTRIM(LTRIM(LAB_TotalAntiHAV));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_TotalAntiHBc'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HEP_B_TOTAL_ANTIBODY = RTRIM(LTRIM(LAB_TotalAntiHBc));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_TotalAntiHDV'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HEP_D_TOTAL_ANTIBODY = RTRIM(LTRIM(LAB_TotalAntiHDV));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'LAB_TotalAntiHEV'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_LAB_FINDING'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_LAB_FINDING set HEP_E_TOTAL_ANTIBODY = RTRIM(LTRIM(LAB_TotalAntiHEV));'
                execute sp_executesql  @sql
            END;
...
----34
        ...
        FROM rdb.dbo.TMP_F_INV_MEDICAL_HISTORY AS F
                 LEFT JOIN	 D_INV_MEDICAL_HISTORY AS D WITH(NOLOCK)  ON F.D_INV_MEDICAL_HISTORY_KEY = D.D_INV_MEDICAL_HISTORY_KEY
        ORDER BY F.INVESTIGATION_KEY;
...
*/
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'MTH_MotherBornOutsideUS'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_MOTHER'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_MOTHER set MTH_BORN_OUTSIDE_US = RTRIM(LTRIM(MTH_MotherBornOutsideUS));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'MTH_MotherEthnicity'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_MOTHER'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_MOTHER set MTH_ETHNICITY = RTRIM(LTRIM(MTH_MotherEthnicity));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'MTH_MotherHBsAgPosPrior'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_MOTHER'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_MOTHER set MTH_HBS_AG_PRIOR_POS = RTRIM(LTRIM(MTH_MotherHBsAgPosPrior));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'MTH_MotherPositiveAfter'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_MOTHER'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_MOTHER set MTH_POS_AFTER = RTRIM(LTRIM(MTH_MotherPositiveAfter));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'MTH_MotherRace'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_MOTHER'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_MOTHER set MTH_RACE = RTRIM(LTRIM(MTH_MotherRace));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'MTH_MothersBirthCountry'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_MOTHER'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_MOTHER set MTH_BIRTH_COUNTRY = RTRIM(LTRIM(MTH_MothersBirthCountry));'
                execute sp_executesql  @sql
            END;
       ...
       */
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_BloodExpOther'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set BLD_EXPOSURE_IND = RTRIM(LTRIM(RSK_BloodExpOther));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_BloodTransfusion'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set BLD_RECVD_IND = RTRIM(LTRIM(RSK_BloodTransfusion));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_BloodWorkerCnctFreq'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set MED_DEN_BLD_CT_FRQ = RTRIM(LTRIM(RSK_BloodWorkerCnctFreq));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_BloodWorkerEver'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set MED_DEN_EMP_EVER_IND = RTRIM(LTRIM(RSK_BloodWorkerEver));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_BloodWorkerOnset'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set MED_DEN_EMPLOYEE_IND = RTRIM(LTRIM(RSK_BloodWorkerOnset));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_ClottingPrior87'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set CLOTFACTOR_PRIOR_1987 = RTRIM(LTRIM(RSK_ClottingPrior87));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_ContaminatedStick'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set BLD_CONTAM_IND = RTRIM(LTRIM(RSK_ContaminatedStick));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_DentalOralSx'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set DEN_WORK_OR_SURG_IND = RTRIM(LTRIM(RSK_DentalOralSx));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_HEMODIALYSIS_BEFORE_ONSET'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set HEMODIALYSIS_IND = RTRIM(LTRIM(RSK_HEMODIALYSIS_BEFORE_ONSET));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_HemodialysisLongTerm'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set LT_HEMODIALYSIS_IND = RTRIM(LTRIM(RSK_HemodialysisLongTerm));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_HospitalizedPrior'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set HSPTL_PRIOR_ONSET_IND = RTRIM(LTRIM(RSK_HospitalizedPrior));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_IDU'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set EVER_INJCT_NOPRSC_DRG = RTRIM(LTRIM(RSK_IDU));'
                execute sp_executesql  @sql
            END;
...
*/
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_Incarcerated24Hrs'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set INCAR_24PLUSHRS_IND = RTRIM(LTRIM(RSK_Incarcerated24Hrs));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_Incarcerated6months'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set INCAR_6PLUS_MO_IND = RTRIM(LTRIM(RSK_Incarcerated6months));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_IncarceratedEver'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set EVER_INCAR_IND = RTRIM(LTRIM(RSK_IncarceratedEver));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_IncarceratedJail'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set INCAR_TYPE_JAIL_IND = RTRIM(LTRIM(RSK_IncarceratedJail));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_IncarcerationPrison'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set INCAR_TYPE_PRISON_IND = RTRIM(LTRIM(RSK_IncarcerationPrison));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_IncarcJuvenileFacilit'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set INCAR_TYPE_JUV_IND = RTRIM(LTRIM(RSK_IncarcJuvenileFacilit));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_IncarcTimeMonths'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set LAST6PLUSMO_INCAR_PER = RTRIM(LTRIM(RSK_IncarcTimeMonths));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_IncarcYear6Mos'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set LAST6PLUSMO_INCAR_YR = RTRIM(LTRIM(RSK_IncarcYear6Mos));'
                execute sp_executesql  @sql
            END;
...
CAST(RSK_IVInjectInfuseOutpt AS varchar(300)) AS OUTPAT_IV_INF_IND, ---22
		...
		*/
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_IVInjectInfuseOutpt'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set OUTPAT_IV_INF_IND = RTRIM(LTRIM(RSK_IVInjectInfuseOutpt));'
                execute sp_executesql  @sql
            END;


        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_LongTermCareRes'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set LTCARE_RESIDENT_IND = RTRIM(LTRIM(RSK_LongTermCareRes));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_NumSexPrtners'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set LIFE_SEX_PRTNR_NBR = RTRIM(LTRIM(RSK_NumSexPrtners));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_OtherBldExpSpec'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set BLD_EXPOSURE_OTH = RTRIM(LTRIM(RSK_OtherBldExpSpec));'
                execute sp_executesql  @sql
            END;

        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_Piercing'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set PIERC_PRIOR_ONSET_IND = RTRIM(LTRIM(RSK_Piercing));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_PiercingOthLocSpec'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set PIERC_PERF_LOC_OTH = RTRIM(LTRIM(RSK_PiercingOthLocSpec));'
                execute sp_executesql  @sql
            END;


        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_PiercingRcvdFrom'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set PIERC_PERF_LOC = RTRIM(LTRIM(RSK_PiercingRcvdFrom));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_PSWrkrBldCnctFreq'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set PUB_SAFETY_BLD_CT_FRQ = RTRIM(LTRIM(RSK_PSWrkrBldCnctFreq));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_PublicSafetyWorker'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set PUB_SAFETY_WORKER_IND = RTRIM(LTRIM(RSK_PublicSafetyWorker));'
                execute sp_executesql  @sql
            END;

        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_STDTxEver'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set STD_TREATED_IND = RTRIM(LTRIM(RSK_STDTxEver));'
                execute sp_executesql  @sql
            END;

        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_STDTxYr'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set STD_LAST_TREATMENT_YR = RTRIM(LTRIM(RSK_STDTxYr));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_SurgeryOther'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set NON_ORAL_SURGERY_IND = RTRIM(LTRIM(RSK_SurgeryOther));'
                execute sp_executesql  @sql
            END;

        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_Tattoo'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set TATT_PRIOR_ONSET_IND = RTRIM(LTRIM(RSK_Tattoo));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_TattooLocation'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set TATTOO_PERF_LOC = RTRIM(LTRIM(RSK_TattooLocation));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_TattooLocOthSpec'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set TATT_PRIOR_LOC_OTH = RTRIM(LTRIM(RSK_TattooLocOthSpec));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_TransfusionPrior92'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set BLD_TRANSF_PRIOR_1992 = RTRIM(LTRIM(RSK_TransfusionPrior92));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_TransplantPrior92'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set ORGN_TRNSP_PRIOR_1992 = RTRIM(LTRIM(RSK_TransplantPrior92));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'RSK_HepContactEver'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_RISK_FACTOR'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_RISK_FACTOR set HEP_CONTACT_EVER_IND = RTRIM(LTRIM(RSK_HepContactEver));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'TRV_HouseholdTravel'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_TRAVEL'))
            BEGIN
                ...
                SET @sql = N'update rdb.dbo.TMP_D_INV_TRAVEL set HOUSEHOLD_TRAVEL_IND = RTRIM(LTRIM(TRV_HouseholdTravel));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'TRV_PatientTravel'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_TRAVEL'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_TRAVEL set TRAVEL_OUT_USACAN_IND = RTRIM(LTRIM(TRV_PatientTravel));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'TRV_PtTravelCountries'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_TRAVEL'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_TRAVEL set TRAVEL_OUT_USACAN_LOC = RTRIM(LTRIM(TRV_PtTravelCountries));'
                execute sp_executesql  @sql
            END;


        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'TRV_TravelCountryHouse'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_TRAVEL'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_TRAVEL set HOUSEHOLD_TRAVEL_LOC = RTRIM(LTRIM(TRV_TravelCountryHouse));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'TRV_VHF_TRAVEL_REASON'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_TRAVEL'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_TRAVEL set TRAVEL_REASON = RTRIM(LTRIM(TRV_VHF_TRAVEL_REASON));'
                execute sp_executesql  @sql
            END;
        ...
        IF NOT  EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'VAC_VaccinationDate'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_VACCINATION'))
            BEGIN
                SET @sql = N'alter table  rdb.dbo.TMP_D_INV_VACCINATION add  VAC_VaccinationDate  varchar(2000) NULL;'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'VAC_ImmuneGlobulin'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_VACCINATION'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_VACCINATION set IMM_GLOB_RECVD_IND = RTRIM(LTRIM(VAC_ImmuneGlobulin));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'VAC_Vacc_Rcvd'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_VACCINATION'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_VACCINATION set VACC_RECVD_IND = RTRIM(LTRIM(VAC_Vacc_Rcvd));'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'VAC_VaccineDoses'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_VACCINATION'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_VACCINATION set VACC_DOSE_RECVD_NBR= (VAC_VaccineDoses);'
                execute sp_executesql  @sql
            END;

        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'VAC_YearofLastDose'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_VACCINATION'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_VACCINATION set VACC_LAST_RECVD_YR = (VAC_YearofLastDose);'
                execute sp_executesql  @sql
            END;
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'VAC_VaccinationDate'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INV_VACCINATION'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INV_VACCINATION set VAC_VaccinationDate = (VAC_VaccinationDate);'
                execute sp_executesql  @sql
            END;
...
------is same as dataset- D_INVESTIGATION_REPEAT
        ...
        IF EXISTS(SELECT 1 FROM sys.columns   WHERE Name = N'VAC_VaccineDoseNum'   AND Object_ID = Object_ID(N'rdb.dbo.TMP_D_INVESTIGATION_REPEAT'))
            BEGIN
                SET @sql = N'update rdb.dbo.TMP_D_INVESTIGATION_REPEAT set VAC_VaccineDoseNum1 = RTRIM(LTRIM(VAC_VaccineDoseNum));'
                execute sp_executesql  @sql
            END;
...
----e
        ...
        FROM rdb.dbo.TMP_Investigation AS I WITH(NOLOCK)
                 FULL OUTER JOIN  rdb.dbo.TMP_D_INV_LAB_FINDING AS L WITH(NOLOCK)  ON I.INVESTIGATION_KEY = L.LAB_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_D_INV_RISK_FACTOR AS R WITH(NOLOCK)  ON I.INVESTIGATION_KEY = R.RISK_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_D_INV_EPIDEMIOLOGY AS E WITH(NOLOCK)  ON I.INVESTIGATION_KEY = E.EPIDEMIOLOGY_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_D_Patient AS P WITH(NOLOCK)          ON I.INVESTIGATION_KEY = P.Patient_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_D_INV_VACCINATION AS V WITH(NOLOCK)  ON I.INVESTIGATION_KEY = V.VACCINATION_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_D_INV_TRAVEL AS T WITH(NOLOCK)  ON I.INVESTIGATION_KEY = T.Travel_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_D_INV_MOTHER AS M WITH(NOLOCK)  ON I.INVESTIGATION_KEY = M.Mother_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_D_INV_MEDICAL_HISTORY AS MH WITH(NOLOCK)  ON I.INVESTIGATION_KEY = MH.MEDHistory_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_D_INV_ADMINISTRATIVE AS A WITH(NOLOCK)  ON I.INVESTIGATION_KEY = A.ADMIN_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_D_INV_PATIENT_OBS AS PO WITH(NOLOCK)  ON I.INVESTIGATION_KEY = PO.PATIENT_OBS_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_HEP_PAT_PROV AS HP WITH(NOLOCK)  ON I.INVESTIGATION_KEY = HP.HEP_PAT_PROV_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_D_INV_CLINICAL AS C WITH(NOLOCK)  ON I.INVESTIGATION_KEY = C.CLINICAL_INV_KEY
                 FULL OUTER JOIN  rdb.dbo.TMP_VAC_REPEAT_OUT_FINAL AS VAC WITH(NOLOCK)  ON I.INVESTIGATION_KEY = VAC.INVESTIGATION_KEY
        ;
...
*/
    ...
    */
    ...
    FROM RDB.DBO.TEMP_HEP_NOTIFICATION WITH (NOLOCK)
    ...
    */
        ...
        INSERT INTO rdb.dbo.[HEPATITIS_DATAMART]( INNC_NOTIFICATION_DT, ---1
        ...
        SET @PROC_STEP_NAME = 'Updating EVENT DATE AND TYPE in RDB.Hepatitis_Case_DATAMART';
        ...
        FROM rdb.dbo.[HEPATITIS_DATAMART] ISD
...
-----------------------------------------------------26e
        ...
        IF OBJECT_ID('rdb.dbo.tmp_inv_sum_nnd_info', 'U') IS NOT NULL
            BEGIN    ------27
        ...
        INSERT INTO rdb.[dbo].[job_flow_log]( batch_id, [Dataflow_Name], [package_Name], [Status_Type], [step_number], [step_name], [row_count] )
        VALUES( @batch_id, 'HEPATITIS_DATAMART', 'RDB.Hepatitis_Case_DATAMART', 'COMPLETE', @Proc_Step_no, @Proc_Step_name, @RowCount_no );
    ...
    BEGIN CATCH
        ...
        VALUES( @Batch_id, 'HEPATITIS_DATAMART', 'RDB.Hepatitis_Case_DATAMART', 'ERROR', @Proc_Step_no, 'ERROR - ' + @Proc_Step_name, 'Step -' + CAST(@Proc_Step_no AS varchar(3)) + ' -' + CAST(@ErrorMessage AS varchar(500)), 0 );

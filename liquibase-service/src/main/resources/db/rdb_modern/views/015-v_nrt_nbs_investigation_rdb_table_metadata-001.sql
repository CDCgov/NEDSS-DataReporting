IF EXISTS(SELECT * FROM sys.views WHERE name = 'v_nrt_nbs_investigation_rdb_table_metadata')
BEGIN
    DROP VIEW [dbo].v_nrt_nbs_investigation_rdb_table_metadata
END
--GO   "GO" not supported by liquibase, keep in manual scripts

CREATE VIEW [dbo].v_nrt_nbs_investigation_rdb_table_metadata 
AS
SELECT DISTINCT 
	page.FORM_CD, 
	page.DATAMART_NM, 

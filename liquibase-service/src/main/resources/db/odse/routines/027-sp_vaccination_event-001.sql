CREATE or ALTER PROCEDURE [dbo].[sp_vaccination_event] @vac_uids nvarchar(max), @debug bit = 'false'
as
BEGIN
 DECLARE
        @RowCount_no INT;
	...
	LEFT JOIN (
        SELECT nq.code_set_group_id, cvg.code, cvg.code_short_desc_txt
        FROM NBS_ODSE.dbo.NBS_question nq
        JOIN NBS_SRTE.dbo.Codeset cd ON cd.code_set_group_id = nq.code_set_group_id
        JOIN NBS_SRTE.dbo.Code_value_general cvg ON cvg.code_set_nm = cd.code_set_nm
        WHERE nq.question_identifier = 'VAC101'
    ) cvg1 ON I.MATERIAL_CD = cvg1.code
    LEFT JOIN (
        SELECT nq.code_set_group_id, cvg.code, cvg.code_short_desc_txt
        FROM NBS_ODSE.dbo.NBS_question nq
        JOIN NBS_SRTE.dbo.Codeset cd ON cd.code_set_group_id = nq.code_set_group_id
    ...
    ) cvg2 ON I.TARGET_SITE_CD = cvg2.code
    LEFT JOIN (
        SELECT nq.code_set_group_id, cvg.code, cvg.code_short_desc_txt
        FROM NBS_ODSE.dbo.NBS_question nq
        JOIN NBS_SRTE.dbo.Codeset cd ON cd.code_set_group_id = nq.code_set_group_id
    ...
    ) cvg3 ON I.AGE_AT_VACC_UNIT_CD = cvg3.code
    LEFT JOIN (
        SELECT nq.code_set_group_id, cvg.code, cvg.code_short_desc_txt
        FROM NBS_ODSE.dbo.NBS_question nq
        JOIN NBS_SRTE.dbo.Codeset cd ON cd.code_set_group_id = nq.code_set_group_id
    ...
    ) cvg4 ON I.VACC_MFGR_CD = cvg4.code
    LEFT JOIN (
        SELECT nq.code_set_group_id, cvg.code, cvg.code_short_desc_txt
        FROM NBS_ODSE.dbo.NBS_question nq
        JOIN NBS_SRTE.dbo.Codeset cd ON cd.code_set_group_id = nq.code_set_group_id
    ...
    ) cvg5 ON I.VACC_INFO_SOURCE_CD = cvg5.code;

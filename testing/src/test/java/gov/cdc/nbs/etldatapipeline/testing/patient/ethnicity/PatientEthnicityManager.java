...
import gov.cdc.nbs.etldatapipeline.testing.patient.ethnicity.PatientEthnicity.Ethnicity;
...
public class PatientEthnicityManager {
  ...
  public void add(final long patient, final PatientEthnicity patientEthnicity) {
    ...
    if (Ethnicity.UNKNOWN.equals(patientEthnicity.ethnicity()) && patientEthnicity.reasonUnknown() != null) {
      reasonUnknown = patientEthnicity.reasonUnknown().code();
    }

    // Set ethnicity values on Person table
    client.sql(SET_ETHNICITY)
    ...
    if (Ethnicity.HISPANIC_OR_LATINO.equals(patientEthnicity.ethnicity())
      ...
      patientEthnicity.spanishOrigins().forEach(so -> {
        client.sql(ADD_SPANISH_ORIGIN)
            .param("patient", patient)

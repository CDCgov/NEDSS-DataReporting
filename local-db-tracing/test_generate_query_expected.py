...
import generate_query_expected
...
class GenerateRdbSelectsTest(unittest.TestCase):
    def test_extracts_declare_block_from_inserts_sql(self) -> None:
        ...
        declare_lines = generate_query_expected.extract_declare_block(inserts_sql)
    ...
    def test_renders_select_using_local_id_variable_when_unambiguous(self) -> None:
        declare_entries = generate_query_expected.parse_declare_entries(
            [
                "DECLARE @dbo_Entity_entity_uid bigint = -1000;",
                "DECLARE @dbo_Person_local_id nvarchar(40) = N'PSN' + CONVERT(nvarchar(20), ABS(CONVERT(bigint, @dbo_Entity_entity_uid))) + N'GA01';",
            ]
        )
        scaffolds = generate_query_expected.build_scaffolds(
            [
                {
                    "schema_name": "dbo",
        ...
        sql, expected_map = generate_query_expected.render_sql(
            {
                "logical_database": "RDB_MODERN",
                "cdc_summary_file": str(generate_query_expected.REPO_ROOT / "summary.txt"),
                "logical_changes_file": str(generate_query_expected.REPO_ROOT / "logical-changes.json"),
            },
            ["DECLARE @dbo_Person_local_id nvarchar(40) = N'PSN' + CONVERT(nvarchar(20), ABS(CONVERT(bigint, @dbo_Entity_entity_uid))) + N'GA01';"],
            declare_entries,
            scaffolds,
    ...
    def test_main_generates_output_file_from_combined_manifest(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            with patch("sys.argv", ["generate_query_expected.py", "--combined-manifest", str(manifest_path)]):
                exit_code = generate_query_expected.main()
    ...
    def test_known_refresh_timestamps_are_excluded_from_select_and_expected_json(self) -> None:
        ...
        sql = generate_query_expected.render_sql(
            {
                "logical_database": "RDB_MODERN",
                "cdc_summary_file": str(generate_query_expected.REPO_ROOT / "summary.txt"),
                "logical_changes_file": str(generate_query_expected.REPO_ROOT / "logical-changes.json"),
    ...
    def test_expected_rows_json_maps_ambiguous_entity_uid_candidates_deterministically(self) -> None:
        ...
        expected_json = generate_query_expected.expected_rows_json(
            (
    ...
    def test_predicate_for_field_cycles_ambiguous_candidates_round_robin(self) -> None:
        ...
        first_predicate, _ = generate_query_expected.predicate_for_field(
            "LOCAL_ID",
            "OBS10001016GA01",
            declare_entries,
            ambiguity_state,
        )
        second_predicate, _ = generate_query_expected.predicate_for_field(
            "LOCAL_ID",
            "OBS10001016GA01",
            declare_entries,
            ambiguity_state,
        )
        third_predicate, _ = generate_query_expected.predicate_for_field(
            "LOCAL_ID",
            "OBS10001016GA01",
            declare_entries,
            ambiguity_state,
    ...
    def test_render_sql_includes_step_comment_for_single_step(self) -> None:
        ...
        sql, _expected_map = generate_query_expected.render_sql(
            {
                "logical_database": "RDB_MODERN",
                "cdc_summary_file": str(generate_query_expected.REPO_ROOT / "summary.txt"),
                "logical_changes_file": str(generate_query_expected.REPO_ROOT / "logical-changes.json"),
    ...
    def test_generate_rdb_selects_writes_cumulative_step_query_files(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            output_path, scaffold_count = generate_query_expected.generate_rdb_selects_from_manifest(manifest_path)

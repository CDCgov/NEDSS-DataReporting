...
SELECT
    ...
    [REPORTING_SOURCE]
FROM [dbo].[CASE_LAB_DATAMART]
WHERE [INVESTIGATION_LOCAL_ID] = N'CAS3300024GA01'
  AND [PATIENT_LOCAL_ID] = N'PSN3300020GA01'
FOR JSON PATH;
...
SELECT
    ...
    [VERSION_CTRL_NBR]
FROM [dbo].[D_CONTACT_RECORD]
WHERE [LOCAL_ID] = N'CON3300027GA01'
FOR JSON PATH;
...
SELECT
    ...
    [RECORD_STATUS_TIME],
    [VERSION_CTRL_NBR]
FROM [dbo].[D_INTERVIEW]
WHERE [LOCAL_ID] = N'INT3300019GA01'
FOR JSON PATH;
...
SELECT
    ...
    [PATIENT_ZIP]
FROM [dbo].[D_PATIENT]
WHERE [PATIENT_LOCAL_ID] = N'PSN3300020GA01'
FOR JSON PATH;
...
SELECT
    ...
    [STATUS_TIME]
FROM [dbo].[EVENT_METRIC]
WHERE [LOCAL_ID] = N'CAS3300024GA01'
FOR JSON PATH;
...
SELECT
    ...
    [STATUS_TIME]
FROM [dbo].[EVENT_METRIC]
WHERE [LOCAL_ID] = N'NOT3300028GA01'
FOR JSON PATH;
...
FROM [dbo].[F_CONTACT_RECORD_CASE]
...
WHERE [INVESTIGATION_LOCAL_ID] = N'CAS3300017GA01'
  AND [NOTIFICATION_LOCAL_ID] = N'NOT3300028GA01'
  AND [PATIENT_LOCAL_ID] = N'PSN3300000GA01'
FOR JSON PATH;
...
SELECT
    ...
    [TRANSMISSION_MODE]
FROM [dbo].[INVESTIGATION]
WHERE [INV_LOCAL_ID] = N'CAS3300024GA01'
FOR JSON PATH;
...
SELECT
    [D_INV_MEDICAL_HISTORY_KEY],
    [PAGE_CASE_UID]
FROM [dbo].[L_INV_MEDICAL_HISTORY]
WHERE [PAGE_CASE_UID] = 3300017.0
FOR JSON PATH;
...
WHERE [D_INVESTIGATION_REPEAT_KEY] IN (1.0, 2.0)
FOR JSON PATH;
...
-- dbo.LOOKUP_TABLE_N_INV_HIV | operations: insert
-- Query: 55
-- Step: 6
-- Identity strategy is fallback_after; review the WHERE clause before using it as a regression assertion.
-- Logical comparison marked this identity as not comparison-safe.
...
-- dbo.LOOKUP_TABLE_N_INV_MEDICAL_HISTORY | operations: insert
-- Query: 56
-- Step: 6
-- Identity strategy is fallback_after; review the WHERE clause before using it as a regression assertion.
-- Logical comparison marked this identity as not comparison-safe.
SELECT
    [D_INV_MEDICAL_HISTORY_KEY],
    [PAGE_CASE_UID]
FROM [dbo].[LOOKUP_TABLE_N_INV_MEDICAL_HISTORY]
WHERE [PAGE_CASE_UID] = 3300017
FOR JSON PATH;
...
-- dbo.LOOKUP_TABLE_N_INV_RISK_FACTOR | operations: insert
-- Query: 58
-- Step: 6
-- Identity strategy is fallback_after; review the WHERE clause before using it as a regression assertion.
-- Logical comparison marked this identity as not comparison-safe.
...
-- dbo.LOOKUP_TABLE_N_INV_SOCIAL_HISTORY | operations: insert
-- Query: 59
-- Step: 6
-- Identity strategy is fallback_after; review the WHERE clause before using it as a regression assertion.
-- Logical comparison marked this identity as not comparison-safe.
SELECT
    [D_INV_SOCIAL_HISTORY_KEY],
    [PAGE_CASE_UID]
FROM [dbo].[LOOKUP_TABLE_N_INV_SOCIAL_HISTORY]
WHERE [PAGE_CASE_UID] = 3300017
FOR JSON PATH;
...
-- dbo.LOOKUP_TABLE_N_REPT | operations: insert
-- Query: 61
-- Step: 9
-- Identity strategy is fallback_after; review the WHERE clause before using it as a regression assertion.
-- Logical comparison marked this identity as not comparison-safe.
...
WHERE [NOTIFICATION_LOCAL_ID] = N'NOT3300028GA01'
FOR JSON PATH;
...
FROM [dbo].[NOTIFICATION_EVENT]
WHERE [CONDITION_KEY] = 44
  AND [INVESTIGATION_KEY] = 3
  AND [NOTIFICATION_KEY] = 2
  AND [NOTIFICATION_SENT_DT_KEY] = 1
  AND [NOTIFICATION_SUBMIT_DT_KEY] = 1
  AND [PATIENT_KEY] = 6
FOR JSON PATH;

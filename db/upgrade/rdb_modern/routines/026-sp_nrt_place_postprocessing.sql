CREATE OR ALTER PROCEDURE dbo.sp_nrt_place_postprocessing
...
SELECT
    k.d_place_key
     ...
     , CASE WHEN LEN(RTRIM(LTRIM(nrt.place_state_desc)))>1 THEN nrt.place_state_desc
    END AS PLACE_STATE_DESC
     , CASE WHEN LEN(RTRIM(LTRIM(nrt.place_county_desc)))>1 THEN nrt.place_county_desc
    ...
    END AS PLACE_COUNTRY_DESC
     ,nrt.place_elp_cd
     ,nrt.place_tele_locator_uid AS 'PLACE_TELE_LOCATOR_UID'
             ,nrt.place_phone_ext     AS 'PLACE_PHONE_EXT'
             ,nrt.place_phone
     ,nrt.place_email
     ,nrt.place_phone_comments
     ,nrt.tele_use_cd
     ,nrt.tele_cd
     ,nrt.place_tele_type
     ,nrt.place_tele_use
     ,CASE
          WHEN LEN(RTRIM(LTRIM(b.first_nm))) > 0 AND LEN(RTRIM(LTRIM(b.last_nm))) > 0 THEN CAST(RTRIM(LTRIM(b.last_nm))+', '+RTRIM(LTRIM(b.first_nm)) AS VARCHAR(102))
          WHEN LEN(RTRIM(LTRIM(b.first_nm))) > 0 AND LEN(RTRIM(LTRIM(b.last_nm))) <= 0 THEN RTRIM(LTRIM(b.first_nm))
          WHEN LEN(RTRIM(LTRIM(b.first_nm))) <= 0 AND LEN(RTRIM(LTRIM(b.last_nm))) > 0 THEN RTRIM(LTRIM(b.last_nm))
          ELSE NULL
    END AS PLACE_ADDED_BY,
    CASE
        WHEN LEN(RTRIM(LTRIM(c.first_nm))) > 0 AND LEN(RTRIM(LTRIM(c.last_nm)))>0 THEN CAST(RTRIM(LTRIM(c.last_nm)) + ', ' +RTRIM(LTRIM(c.first_nm))AS VARCHAR(102))
        WHEN LEN(RTRIM(LTRIM(c.first_nm))) > 0 AND LEN(RTRIM(LTRIM(c.last_nm))) <= 0 THEN RTRIM(LTRIM(c.first_nm))
        WHEN LEN(RTRIM(LTRIM(c.first_nm))) <= 0 AND LEN(RTRIM(LTRIM(c.last_nm))) > 0 THEN RTRIM(LTRIM(c.last_nm))
        ELSE NULL
        END  AS PLACE_LAST_UPDATED_BY
INTO #tmp_place_table
FROM dbo.nrt_place nrt
         LEFT JOIN dbo.nrt_place_key k on k.place_uid = nrt.place_uid
...
BEGIN TRANSACTION;
        ...
        /*Composite PLACE_LOCATOR_UID Generator: To reference PLACE_LOCATOR_UID for both insert and update. */
SELECT DISTINCT
    nrt.place_uid
              , CAST(CAST(nrt.place_uid AS VARCHAR(10))+'^^' AS VARCHAR(30)) as base_uid
              , CASE WHEN nrt.place_postal_uid IS NOT NULL THEN CAST(CAST(nrt.place_uid AS VARCHAR(10))+'^'+CAST(nrt.place_postal_uid AS VARCHAR(10))+'^' AS VARCHAR(30))
                     ELSE NULL END AS postal_base_uid
              , CASE WHEN nrt.place_tele_locator_uid IS NOT NULL THEN CAST(CAST(nrt.place_uid AS VARCHAR(10))+'^^'+CAST(nrt.place_tele_locator_uid AS VARCHAR(10))AS VARCHAR(30))
                     ELSE NULL END AS tele_base_uid
              , CASE WHEN nrt.place_postal_uid IS NOT NULL AND nrt.place_tele_locator_uid IS NOT NULL THEN CAST(CAST(nrt.place_uid AS VARCHAR(10))+'^'+CAST(nrt.place_postal_uid AS VARCHAR(10))+'^'+CAST(nrt.place_tele_locator_uid AS VARCHAR(10))AS VARCHAR(30))
                     ELSE NULL END AS agg_base_uid
INTO #place_locator_ref
FROM #tmp_place_table nrt;


IF @debug = 'true' SELECT * FROM #place_locator_ref;
...
FROM (
         ...
         SELECT DISTINCT
             CASE
                 WHEN s.place_locator_uid IS NULL THEN r.base_uid
                 WHEN s.place_locator_uid = r.base_uid THEN s.place_locator_uid
                 END AS PLACE_LOCATOR_UID,
         ...
         SELECT DISTINCT
             CASE
                 WHEN s.place_locator_uid IS NULL THEN r.postal_base_uid
                 WHEN s.place_locator_uid = r.postal_base_uid THEN s.place_locator_uid
                 END AS PLACE_LOCATOR_UID,
             NULLIF(s.d_place_key, NULL) AS PLACE_KEY,
             S.PLACE_ADD_TIME,
             S.PLACE_ADD_USER_ID,
             S.PLACE_ADDED_BY,
         ...
         FROM  #tmp_place_table s
                   LEFT JOIN #place_locator_ref r on r.place_uid = s.place_uid
                   LEFT JOIN dbo.nrt_place_key k on k.place_locator_uid = r.postal_base_uid
         WHERE (s.place_locator_uid IS NULL
             OR r.postal_base_uid = s.place_locator_uid) AND s.place_postal_uid IS NOT NULL
         UNION ALL
         --Tele: Only Tele Information
         SELECT DISTINCT
             CASE
                 WHEN s.place_locator_uid IS NULL THEN r.tele_base_uid
                 WHEN s.place_locator_uid = r.tele_base_uid THEN s.place_locator_uid
                 END AS PLACE_LOCATOR_UID,
             NULLIF(s.d_place_key, NULL) AS PLACE_KEY,
             S.PLACE_ADD_TIME,
             S.PLACE_ADD_USER_ID,
         ...
         FROM  #tmp_place_table s
                   LEFT JOIN #place_locator_ref r on r.place_uid = s.place_uid
                   LEFT JOIN dbo.nrt_place_key k on k.place_locator_uid = r.tele_base_uid
         WHERE (s.place_locator_uid IS NULL
             OR r.tele_base_uid = s.place_locator_uid) AND s.place_tele_locator_uid IS NOT NULL
         UNION ALL
         --All
         SELECT DISTINCT
             CASE
                 WHEN s.place_locator_uid IS NULL THEN r.agg_base_uid
                 WHEN s.place_locator_uid = r.agg_base_uid THEN s.place_locator_uid
                 END AS PLACE_LOCATOR_UID,
             NULLIF(s.d_place_key, NULL) AS PLACE_KEY,
             S.PLACE_ADD_TIME,
             S.PLACE_ADD_USER_ID,
         ...
         FROM  #tmp_place_table s
                   LEFT JOIN #place_locator_ref r on r.place_uid = s.place_uid
                   LEFT JOIN dbo.nrt_place_key k on r.agg_base_uid	= k.place_locator_uid
         WHERE (s.place_locator_uid IS NULL
             OR r.agg_base_uid = s.place_locator_uid)
           AND ( s.place_tele_locator_uid IS NOT NULL AND s.place_postal_uid IS NOT NULL)
     ) src;
...
WHERE k.d_place_key IS NULL and tmp.place_locator_uid IS NOT NULL;

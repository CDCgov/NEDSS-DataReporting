IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_ldf_mumps_datamart_postprocessing]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_ldf_mumps_datamart_postprocessing]
END
GO 

CREATE PROCEDURE [dbo].[sp_ldf_mumps_datamart_postprocessing] 
...
@debug bit = 'false'
 as 
  BEGIN 
	...
	declare @dataflow_name varchar(200) = 'sp_ldf_mumps_datamart_postprocessing POST-Processing';
	declare @package_name varchar(200) = 'sp_ldf_mumps_datamart_postprocessing';
	...
	DECLARE  @dynamiccolumnList varchar(max)=''	--insert into LDF_MUMPS table from TMP_MUMPS table 
	DECLARE @count BIGINT; 
	

	DECLARE @global_tmp_mumps_ta NVARCHAR(MAX);
	set @global_tmp_mumps_ta = '##TMP_MUMPS_TA_' + CAST(@batch_id as varchar(50));

	DECLARE @global_tmp_mumps_short_col NVARCHAR(MAX);
	set @global_tmp_mumps_short_col = '##TMP_MUMPS_SHORT_COL_' + CAST(@batch_id as varchar(50));

	DECLARE @global_tmp_mumps NVARCHAR(MAX);
	set @global_tmp_mumps = '##TMP_MUMPS_' + CAST(@batch_id as varchar(50));

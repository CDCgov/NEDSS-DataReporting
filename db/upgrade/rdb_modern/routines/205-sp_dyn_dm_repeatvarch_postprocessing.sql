CREATE or ALTER  PROCEDURE [dbo].sp_dyn_dm_repeatvarch_postprocessing
            @batch_id BIGINT,
...
BEGIN TRY
	...
	DECLARE @tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR varchar(500) = '[dbo].tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR_'+ @DATAMART_NAME +'_'+ cast(@batch_id as varchar);
...
--------------------------------------------------------------------------------------------------------------------------------------------
	...
	SET @Proc_Step_Name = ' GENERATING #tmp_DynDm_METADATA_INIT';
 	...
 	FROM dbo.V_NRT_NBS_REPEATVARCH_RDB_TABLE_METADATA meta
...
--------------------------------------------------------------------------------------------------------------------------------------------
	...
	SET @Proc_Step_Name = ' GENERATING  tmp_DynDm_METADATA_UNIT';
 	...
 	SELECT  distinct
 		DATAMART_NM,
		substring(LTRIM(RTRIM(RDB_COLUMN_NM)),1,22)+'_OTH' as RDB_COLUMN_NM,
    ...
    FROM dbo.V_NRT_NBS_REPEATVARCH_RDB_TABLE_METADATA meta
	WHERE INVESTIGATION_FORM_CD = @nbs_page_form_cd
		and (code_set_group_id >0 OR data_type in ( 'Coded','CODED') )
...
--------------------------------------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------------------------------------
--  building tmp_DynDm_BLOCK_DATA from tmp_DynDm_BLOCK_DATA_OTH and tmp_DynDm_BLOCK_DATA_UNIT
----------------------------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if @columns = ',' OR @columns IS NULL
    	...
    	begin
		    ...
		    ' into '+@tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR +
		    ' FROM (
...
--------------------------------------------------------------------------------------------------------------------------------------------
	...
	IF OBJECT_ID(@tmp_DynDm_REPEAT_BLOCK_VARCHAR_ALL, 'U') IS NOT NULL
		exec ('drop table '+ @tmp_DynDm_REPEAT_BLOCK_VARCHAR_ALL);
    ...
    if @columns = ',' OR @columns IS NULL
	    ...
	    begin
		    SET @sql = N'
			    SELECT [INVESTIGATION_KEY] , '+STUFF(@columns, 1, 2, '')+
			    ' into ' + @tmp_DynDm_REPEAT_BLOCK_VARCHAR_ALL +
			    ' FROM (

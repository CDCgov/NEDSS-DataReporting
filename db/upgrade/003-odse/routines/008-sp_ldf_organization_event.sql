IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_ldf_organization_event]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_ldf_organization_event]
END
GO 

CREATE PROCEDURE dbo.sp_ldf_organization_event @ldf_uid_list nvarchar(max), @bus_obj_uid_list nvarchar(max)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        from  dbo.State_Defined_Field_MetaData m
                  join dbo.State_Defined_Field_Data d with (nolock) on m.ldf_uid = d.ldf_uid  and d.business_object_nm = 'ORG'
            ...
            and d.ldf_uid in (SELECT value FROM STRING_SPLIT(@ldf_uid_list, ','))
                  ...
                  join dbo.Organization o with (nolock) on d.business_object_uid=o.organization_uid

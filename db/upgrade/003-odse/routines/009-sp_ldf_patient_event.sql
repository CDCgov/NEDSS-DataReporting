IF EXISTS (SELECT * FROM sysobjects WHERE  id = object_id(N'[dbo].[sp_ldf_patient_event]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
BEGIN
    DROP PROCEDURE [dbo].[sp_ldf_patient_event]
END
GO 

CREATE PROCEDURE dbo.sp_ldf_patient_event @ldf_uid_list nvarchar(max), @bus_obj_uid_list nvarchar(max)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        from  dbo.State_Defined_Field_MetaData m
                  join dbo.State_Defined_Field_Data d with (nolock) on m.ldf_uid = d.ldf_uid  and d.business_object_nm = 'PAT'
            ...
            and d.ldf_uid in (SELECT value FROM STRING_SPLIT(@ldf_uid_list, ','))
                  ...
                  join dbo.Person p with (nolock) on d.business_object_uid=p.person_uid  and p.person_uid<>p.person_parent_uid
            and p.cd='PAT'

...
class LogicalMarkdownTest(unittest.TestCase):
    def test_renders_summary_and_changes(self) -> None:
        ...
        self.assertIn("| Source actions | Added a vaccine |", markdown)
        self.assertIn("| Table | Change count |", markdown)
        self.assertIn("| dbo.Person | 2 |", markdown)
        self.assertIn("## 1. INSERT dbo.Person", markdown)
        self.assertIn("## 2. UPDATE dbo.Person", markdown)
        self.assertIn("| Identity | business_keys: local_id=\"PSN10067007GA01\" |", markdown)
        self.assertIn("| Transaction end | 2026-04-07T14:11:05.230 |", markdown)
        self.assertIn("| LSN | 0x01 |", markdown)

...
Source list: `STRATEGY.md` "Follow-on / phase-2" → teammate's note about ~80
RDB tables with no known ODSE-input path. Cross-referenced against
`coverage/coverage_merged.md` (current empty/partial/full state) and
`coverage/coverage_tier_3.md` (second-pass triage classifications).
...
4. Cross-referenced classifications with `coverage_tier_3.md` and
   adopted its analysis where applicable. **Two corrections** to that
   file's claims, see Notable findings.

## Summary counts

- (a) MasterETL-only (no RTR writer): **22 tables**
- (b) Datamart-SP-driven (Merge step 9 / dyn_dm chain): **38 tables**
- (c) Tier-1-or-Tier-2 reachable (fixture gap): **34 tables**
...
These populate when the orchestrator runs Step 9 (datamart SPs). Most
condition-specific datamarts are blocked today by the TMP_F_PAGE_CASE
transaction-isolation bug (see `coverage_tier_3.md`), not by missing
fixtures.
...
answers — explicit Phase 2 work per `coverage_tier_3.md` "TB-PAM
cluster" finding.
...
| `etl_dq_log` | `007-sp_s_pagebuilder_postprocessing`, `010-sp_sld_investigation_repeat_postprocessing`, `025-sp_f_std_page_case_postprocessing` | **Correction**: `coverage_tier_3.md` claims this is MasterETL-only. It is NOT — three RTR routines INSERT into it as DQ-failure side-channel logging. Will populate once a fixture hits an invalid numeric / DQ-fail branch. |
| `lookup_table_n_rept` | `010-sp_sld_investigation_repeat_postprocessing` | **Correction**: `coverage_tier_3.md` claims MasterETL-only. It is NOT — `sp_sld_investigation_repeat_postprocessing` does `DELETE FROM dbo.LOOKUP_TABLE_N_REPT; INSERT INTO dbo.LOOKUP_TABLE_N_REPT …` (lines 1144-1146). Will populate once the SLD repeat chain runs against a fixture with NBS_PAGE rows. |
...
1. **Two MasterETL-only claims in `coverage_tier_3.md` are wrong.**
   That file says "`etl_dq_log`, `lookup_table_n_rept`: NO writer in
   any RTR routine — MasterETL-only tables." Both are in fact written
   by RTR routines. `etl_dq_log` receives DQ-failure rows from three
   page-builder / SLD-repeat SPs (DQ_ISSUE_DESC_TXT, etc.) — bucket
   (c). `lookup_table_n_rept` is fully rewritten by
   `010-sp_sld_investigation_repeat_postprocessing` (DELETE +
   INSERT) — also bucket (c). This is good news: two more tables
   reachable than previously thought, but only if a fixture
   exercises the SLD repeat chain or a DQ-fail branch.
...
2. **The `dm_inv_*` family is almost certainly a misnamed
   ...
   *does* build `DM_INV_<datamart_nm>` tables dynamically via
   ...
   dyn_dm chain. The eventual RDB-vs-RDB_MODERN diff will surface
   this as a **schema-shape gap**, not a row-count gap.
...
The multi-condition agent owners should validate against this list as
they extend the fan-out fixtures.

## Recommended Phase 2 priority

1. **First**: author the TB-PAM full chain fixture (TB Investigation
   with RVCT form `nbs_case_answer` rows). 26 of 34 bucket (c) tables
   light up from a single fixture file. This is the highest yield-per-
   effort opportunity in the catalog.
2. **Second**: fix the **TMP_F_PAGE_CASE transaction-isolation bug**
   in the condition-datamart SP family (see `coverage_tier_3.md`).
   Without this, all bucket (b) condition-specific datamarts stay at
...
5. **Skip for now**: pure MasterETL-only tables (`geocoding_location`,
   ...
   RDB_MODERN doesn't" in the diff — that's a *finding* for the
   comparison report, not a fixture authoring task.

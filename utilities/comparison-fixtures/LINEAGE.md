...
> closed that. The per-cluster slices live in `lineage/sections/` and
> `lineage/columns/` and the TSV is regenerated by concatenating them in
> order L1–L6, G1–G4. **Do not re-run the fan-out below** — the instructions
> are retained as a record of method. Extend a slice + regenerate the TSV if
> the SP set changes.

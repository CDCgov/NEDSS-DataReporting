...
-- =====================================================================
INSERT INTO [dbo].[nrt_treatment]
    ( [treatment_uid], [organization_uid], [provider_uid]                    -- 01-03
    , [patient_treatment_uid], [morbidity_uid], [treatment_name]             -- 04-06
    , [treatment_oid], [treatment_comments], [treatment_shared_ind]          -- 07-09
    , [cd], [treatment_date], [treatment_drug]                               -- 10-12
    , [treatment_drug_name], [treatment_dosage_strength]                     -- 13-14
    , [treatment_dosage_strength_unit], [treatment_frequency]                -- 15-16
    , [treatment_duration], [treatment_duration_unit], [treatment_route]     -- 17-19
    , [local_id], [record_status_cd]                                         -- 20-21
    , [add_time], [add_user_id], [last_change_time]                          -- 22-24
    , [last_change_user_id], [version_control_number]                        -- 25-26
    , [associated_phc_uids]                                                  -- 27
    )
VALUES
    -- ---------------------------------------------------------------
    -- Row 1. Foundation Treatment (UID 20000150) — sparse / null-propagation.
    --   organization_uid / provider_uid / patient_treatment_uid /
    --   morbidity_uid / associated_phc_uids all NULL (no participation
    --   rows on the foundation Treatment, so the upstream
    --   person-service / Debezium projection produces NULLs for these
    --   soft refs). Drug / dose / frequency / duration / route NULL.
    --   Drives null path on every TREATMENT-dim column the
    --   postprocessing SP populates from nrt_treatment.
    -- ---------------------------------------------------------------
    ( @foundation_act_treatment_uid, NULL, NULL                              -- 01-03
    , NULL, NULL, NULL                                                       -- 04-06
    , NULL, NULL, N'F'                                                       -- 07-09
    , N'TRMT100', NULL, NULL                                                 -- 10-12
    , NULL, NULL                                                             -- 13-14
    , NULL, NULL                                                             -- 15-16
    , NULL, NULL, NULL                                                       -- 17-19
    , N'TRT20000150GA01', N'ACTIVE'                                          -- 20-21
    , '2026-04-01T00:00:00', @superuser_id, '2026-04-01T00:00:00'            -- 22-24
    , N'10009282', N'1'                                                      -- 25-26
    , NULL                                                                   -- 27
    ),
    -- ---------------------------------------------------------------
    -- Row 2. v2 Treatment (UID 20100010) — fully populated.
    --   Cross-subject soft refs point at foundation Patient / Provider
    --   / Organization / Morbidity / Investigation. The postprocessing
    --   SP COALESCEs each of these to sentinel 1 (lines 184-193), so
    --   at Tier 1 isolation TREATMENT_EVENT FK-key columns end up with
    --   sentinel 1 (D_PATIENT, D_ORGANIZATION, D_PROVIDER are empty
    --   for these UIDs in baseline; merged-fixture sequence resolves
    --   to real keys after upstream chains run).
    --   cd='1' is a real TREAT_COMPOSITE code (per nrt_srte_Treatment_code
    --   seed). cd is NOT 'OTH' so CUSTOM_TREATMENT resolves to NULL
    --   for v2 (the ELSE branch of the SP's CASE at line 88).
    -- ---------------------------------------------------------------
    ( @dbo_Act_treatment_v2_uid, @foundation_org_uid, @foundation_provider_uid -- 01-03
    , @foundation_patient_uid, @foundation_morb_uid                          -- 04-05
    , N'Acyclovir, 200 mg, PO, 5ID, x 5 days'                                -- 06
    , N'20100010'                                                            -- 07 (treatment_oid is varchar in nrt_treatment though TREATMENT.TREATMENT_OID is bigint; SP passes through as-is)
    , N'Tier 1 Treatment v2 — clinician comments on therapy course.', N'T'   -- 08-09
    , N'1', '2026-04-02T08:00:00', N'500'                                    -- 10-12
    , N'Acyclovir', N'200'                                                   -- 13-14
    , N'mg', N'TID'                                                          -- 15-16
    , N'5', N'D', N'C0205531'                                                -- 17-19
    , N'TRT20100010GA01', N'ACTIVE'                                          -- 20-21
    , '2026-04-01T00:00:00', @superuser_id, '2026-04-01T00:00:00'            -- 22-24
    , N'10009282', N'1'                                                      -- 25-26
    , CAST(@foundation_investigation_uid AS nvarchar(20))                    -- 27 -- single PHC; the SP at lines 130/134 splits the CSV via OUTER APPLY string_split
    ),
    -- ---------------------------------------------------------------
    -- Row 3. v3 Treatment (UID 20100020) — cd='OTH' free-text variant.
    --   Exercises the SP's CUSTOM_TREATMENT CASE branch (line 88):
    --     WHEN nrt.cd = 'OTH' THEN nrt.treatment_name -> CUSTOM_TREATMENT
    --   treatment_name is non-NULL and surfaces as CUSTOM_TREATMENT
    --   in the TREATMENT dim. Other clinical detail columns NULL —
    --   v2 already covers their populated path. associated_phc_uids
    --   NULL — exercises the no-PHC path on a separate row from
    --   foundation (foundation also has NULL associated_phc_uids).
    -- ---------------------------------------------------------------
    ( @dbo_Act_treatment_v3_uid, NULL, NULL                                  -- 01-03
    , NULL, NULL, N'Free-text custom treatment plan'                         -- 04-06
    , NULL, NULL, N'F'                                                       -- 07-09
    , N'OTH', NULL, NULL                                                     -- 10-12
    , NULL, NULL                                                             -- 13-14
    , NULL, NULL                                                             -- 15-16
    , NULL, NULL, NULL                                                       -- 17-19
    , N'TRT20100020GA01', N'ACTIVE'                                          -- 20-21
    , '2026-04-01T00:00:00', @superuser_id, '2026-04-01T00:00:00'            -- 22-24
    , N'10009282', N'1'                                                      -- 25-26
    , NULL                                                                   -- 27
    );

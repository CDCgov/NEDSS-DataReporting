...
INSERT INTO [dbo].[nrt_investigation_notification]
    ([source_act_uid], [public_health_case_uid], [source_class_cd],
     [target_class_cd], [act_type_cd], [status_cd], [notification_uid],
     [prog_area_cd], [program_jurisdiction_oid], [jurisdiction_cd],
     [record_status_time], [status_time], [rpt_sent_time],
     [notif_status], [notif_local_id], [notif_comments], [notif_add_time],
     [notif_add_user_id], [notif_add_user_name],
     [notif_last_chg_user_id], [notif_last_chg_user_name],
     [notif_last_chg_time], [local_patient_id], [local_patient_uid],
     [condition_cd], [condition_desc],
     [first_notification_status], [notif_rejected_count],
     [notif_created_count], [notif_sent_count],
     [first_notification_send_date], [notif_created_pending_count],
     [last_notification_date], [last_notification_send_date],
     [first_notification_date], [first_notification_submitted_by],
     [last_notification_submitted_by], [notification_date])
VALUES
-- Foundation Notification (UID 20000110): deliberately NULL on the
-- columns the SP propagates that aren't required to drive the FK joins.
-- This exhibits the SP's null/blank transform path:
--   NOTIFICATION_STATUS / NOTIFICATION_COMMENTS / NOTIFICATION_LOCAL_ID
--   / NOTIFICATION_SUBMITTED_BY / NOTIFICATION_LAST_CHANGE_TIME each
--   propagate as the staging value (which is NULL for the all-NULL
--   columns). NOTIFICATION_LOCAL_ID is the only one populated below
--   because foundation's `notification.local_id` is 'NOT20000110GA01'.
--   public_health_case_uid is set to foundation Investigation so the
--   INVESTIGATION join resolves (when INVESTIGATION dim has the row,
--   which Investigation Tier 1 has populated only when its fixture
--   runs; at Notification-isolation time it is empty so the COALESCE
--   sentinel kicks in).
    (@foundation_act_notif_uid,         -- 1  source_act_uid (= notification_uid)
     @foundation_act_inv_uid,            -- 2  public_health_case_uid (foundation Investigation; soft cross-subject UID — no act_relationship row authored)
     N'NOTF',                            -- 3  source_class_cd
     N'CASE',                            -- 4  target_class_cd
     N'Notification',                    -- 5  act_type_cd (canonical PAR_TYPE 'Notification' — informational; not read by postprocessing SP)
     N'A',                               -- 6  status_cd
     @foundation_act_notif_uid,          -- 7  notification_uid (driver key)
     N'STD',                             -- 8  prog_area_cd (matches foundation notification.prog_area_cd)
     NULL,                               -- 9  program_jurisdiction_oid
     N'1',                               -- 10 jurisdiction_cd (matches foundation notification.jurisdiction_cd)
     '2026-04-01T00:00:00',              -- 11 record_status_time
     '2026-04-01T00:00:00',              -- 12 status_time
     NULL,                               -- 13 rpt_sent_time (foundation: NULL — exhibits NOTIFICATION_SENT_DT_KEY=COALESCE(NULL,1)=1 path)
     NULL,                               -- 14 notif_status (foundation: NULL — exhibits NOTIFICATION_STATUS NULL propagation)
     N'NOT20000110GA01',                 -- 15 notif_local_id (matches foundation notification.local_id)
     NULL,                               -- 16 notif_comments (foundation notification.txt is NULL; NULL propagates)
     '2026-04-01T00:00:00',              -- 17 notif_add_time (drives NOTIFICATION_SUBMIT_DT_KEY)
     @superuser_id,                      -- 18 notif_add_user_id (drives NOTIFICATION_SUBMITTED_BY)
     N'Foundation, Superuser',           -- 19 notif_add_user_name
     @superuser_id,                      -- 20 notif_last_chg_user_id
     N'Foundation, Superuser',           -- 21 notif_last_chg_user_name
     '2026-04-01T00:00:00',              -- 22 notif_last_chg_time (drives NOTIFICATION_LAST_CHANGE_TIME and NOTIFICATION_UPD_DT_KEY)
     N'PSN20000000GA01',                 -- 23 local_patient_id (matches foundation patient local_id)
     @foundation_patient_uid,            -- 24 local_patient_uid (foundation Patient; soft cross-subject UID — no participation row authored)
     N'10110',                           -- 25 condition_cd (must resolve to a CONDITION row — SP body does NOT COALESCE CONDITION_KEY despite the prompt's expectation; we author a single CONDITION row above and both staging rows reference it. Foundation Notification's null/blank coverage path is exhibited via the other propagated columns instead.)
     NULL,                               -- 26 condition_desc
     NULL,                               -- 27 first_notification_status
     NULL,                               -- 28 notif_rejected_count
     NULL,                               -- 29 notif_created_count
     NULL,                               -- 30 notif_sent_count
     NULL,                               -- 31 first_notification_send_date
     NULL,                               -- 32 notif_created_pending_count
     NULL,                               -- 33 last_notification_date
     NULL,                               -- 34 last_notification_send_date
     NULL,                               -- 35 first_notification_date
     NULL,                               -- 36 first_notification_submitted_by
     NULL,                               -- 37 last_notification_submitted_by
     NULL),                              -- 38 notification_date
-- v2 Notification (UID 20060010): every column the SP reads is populated.
-- This drives the populated path on every NOTIFICATION + NOTIFICATION_EVENT
-- column.
    (@dbo_Act_notification_v2_uid,       -- 1  source_act_uid
     @foundation_act_inv_uid,            -- 2  public_health_case_uid (foundation Investigation; same target as foundation row's notification)
     N'NOTF',                            -- 3  source_class_cd
     N'CASE',                            -- 4  target_class_cd
     N'Notification',                    -- 5  act_type_cd
     N'A',                               -- 6  status_cd
     @dbo_Act_notification_v2_uid,       -- 7  notification_uid
     N'HEP',                             -- 8  prog_area_cd
     20060010,                           -- 9  program_jurisdiction_oid
     N'130001',                          -- 10 jurisdiction_cd
     '2026-04-04T00:00:00',              -- 11 record_status_time
     '2026-04-04T00:00:00',              -- 12 status_time
     '2026-04-04T00:00:00',              -- 13 rpt_sent_time (drives NOTIFICATION_SENT_DT_KEY)
     N'COMPLETED',                       -- 14 notif_status (REC_STAT 'COMPLETED' — verified)
     N'NOT20060010GA01',                 -- 15 notif_local_id
     N'Tier 1 v2 notification comments — exercises NOTIFICATION_COMMENTS.', -- 16 notif_comments (drives NOTIFICATION_COMMENTS)
     '2026-04-01T00:00:00',              -- 17 notif_add_time (drives NOTIFICATION_SUBMIT_DT_KEY)
     @superuser_id,                      -- 18 notif_add_user_id (drives NOTIFICATION_SUBMITTED_BY)
     N'Foundation, Superuser',           -- 19 notif_add_user_name
     @superuser_id,                      -- 20 notif_last_chg_user_id
     N'Foundation, Superuser',           -- 21 notif_last_chg_user_name
     '2026-04-04T00:00:00',              -- 22 notif_last_chg_time (drives NOTIFICATION_LAST_CHANGE_TIME and NOTIFICATION_UPD_DT_KEY)
     N'PSN20000000GA01',                 -- 23 local_patient_id
     @foundation_patient_uid,            -- 24 local_patient_uid (foundation Patient; same as foundation row)
     N'10110',                           -- 25 condition_cd (Hepatitis A, acute — drives CONDITION_KEY join when CONDITION dim is populated)
     N'Hepatitis A, acute',              -- 26 condition_desc
     N'COMPLETED',                       -- 27 first_notification_status
     0,                                  -- 28 notif_rejected_count
     1,                                  -- 29 notif_created_count
     1,                                  -- 30 notif_sent_count
     '2026-04-04T00:00:00',              -- 31 first_notification_send_date
     0,                                  -- 32 notif_created_pending_count
     '2026-04-04T00:00:00',              -- 33 last_notification_date
     '2026-04-04T00:00:00',              -- 34 last_notification_send_date
     '2026-04-01T00:00:00',              -- 35 first_notification_date
     @superuser_id,                      -- 36 first_notification_submitted_by
     @superuser_id,                      -- 37 last_notification_submitted_by
     '2026-04-04T00:00:00');             -- 38 notification_date

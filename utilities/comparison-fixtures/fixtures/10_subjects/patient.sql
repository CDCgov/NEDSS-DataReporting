...
INSERT INTO [dbo].[nrt_patient]
    ([patient_uid], [patient_mpr_uid], [record_status], [local_id],
     ...
     [nm_use_cd], [status_name_cd], [alias_nickname],
     [street_address_1], [street_address_2], [city], [state], [state_code],
     [zip], [county], [county_code], [country], [country_code],
     [within_city_limits],
     [phone_home], [phone_ext_home], [phone_work], [phone_ext_work],
     [phone_cell], [email],
     [dob], [age_reported], [age_reported_unit], [age_reported_unit_cd],
     [birth_sex], [current_sex], [curr_sex_cd],
     [deceased_indicator], [deceased_ind_cd], [deceased_date],
     [marital_status], [ssn], [ethnic_group_ind], [ethnicity],
     [race_calculated], [race_calc_details],
     [race_amer_ind_1], [race_amer_ind_2], [race_amer_ind_3], [race_amer_ind_gt3_ind], [race_amer_ind_all],
     [race_asian_1], [race_asian_2], [race_asian_3], [race_asian_gt3_ind], [race_asian_all],
     [race_black_1], [race_black_2], [race_black_3], [race_black_gt3_ind], [race_black_all],
     [race_nat_hi_1], [race_nat_hi_2], [race_nat_hi_3], [race_nat_hi_gt3_ind], [race_nat_hi_all],
     [race_white_1], [race_white_2], [race_white_3], [race_white_gt3_ind], [race_white_all],
     [patient_number], [patient_number_auth],
     [entry_method], [speaks_english], [unk_ethnic_rsn], [curr_sex_unk_rsn],
     [preferred_gender], [addl_gender_info], [census_tract], [race_all],
     [birth_country], [primary_occupation], [primary_language],
     [add_user_id], [add_user_name], [add_time],
     [last_chg_user_id], [last_chg_user_name], [last_chg_time])
VALUES
-- Foundation Patient (UID 20000000): minimal but valid row. Mirrors the
-- foundation ODSE row enriched above. NULLs left on a few optional cols
-- so the SP's null/blank-handling path is exercised.
    (20000000, NULL, N'ACTIVE', N'PSN20000000GA01',
     NULL,                                              -- general_comments NULL (deliberate)
     N'Foundation', NULL, N'Patient', NULL,             -- middle_name + name_suffix NULL (deliberate)
     N'L', N'A', NULL,                                  -- alias_nickname NULL (deliberate)
     N'100 Foundation Way', NULL, N'Atlanta', N'Georgia', N'13',
     N'30303', N'Fulton County', N'13121', N'United States', N'840',
     NULL,                                              -- within_city_limits NULL (deliberate)
     N'404-555-0100', NULL, NULL, NULL,                 -- phone_work, ext NULL (deliberate)
     NULL, N'foundation.patient@nbs.test',              -- phone_cell NULL (deliberate)
     '1990-01-15T00:00:00', NULL, NULL, NULL,           -- age_reported NULL (deliberate; uses dob path)
     N'Male', N'Male', N'M',
     N'No', N'N', NULL,                                 -- deceased_date NULL (deliberate; alive)
     NULL, NULL, N'2186-5', N'Not Hispanic or Latino',  -- marital_status, ssn NULL (deliberate)
     N'White', N'White',                                -- single-root race
     NULL, NULL, NULL, NULL, NULL,
     NULL, NULL, NULL, NULL, NULL,
     NULL, NULL, NULL, NULL, NULL,
     NULL, NULL, NULL, NULL, NULL,
     NULL, NULL, NULL, NULL, NULL,
     N'PAT-FND-1', N'Patient Internal Identifier',
     N'ELECTRONIC', NULL, NULL, NULL,                   -- speaks_english/unk reasons NULL (deliberate)
     NULL, NULL,                                        -- preferred_gender, addl_gender NULL (deliberate)
     NULL, N'White',                                    -- census_tract NULL (deliberate)
     N'CHINA', NULL, NULL,                              -- primary_occupation, primary_language NULL (deliberate)
     10009282, N'Kent, Ariella', '2026-04-01T00:00:00',
     10009282, N'Kent, Ariella', '2026-04-01T00:00:00'),

-- v2 Patient (UID 20020010): every column populated to maximize D_PATIENT
-- coverage. Race profile: American Indian + Asian (Multi-Race).
    (20020010, 20020010, N'ACTIVE', N'PSN20020010GA01',
     N'Tier 1 variant patient — exercises every D_PATIENT column.',
     N'Variant', N'Marie', N'Patient', N'Jr.',
     N'L', N'A', N'Vee',
     N'500 Variant Patient Lane', N'Apartment 7B', N'Atlanta', N'Georgia', N'13',
     N'30303', N'Fulton County', N'13121', N'United States', N'840',
     N'Y',
     N'404-555-2010', N'4321', N'404-555-2011', N'9999',
     N'404-555-2012', N'variant.patient@nbs.test',
     '1985-06-15T00:00:00', 40, N'Years', N'Y',
     N'Female', N'Female', N'F',
     N'No', N'N', NULL,                                 -- alive — deceased_date NULL
     N'Married', N'987-65-4321', N'2135-2', N'Hispanic or Latino',
     N'Multi-Race',
     N'American Indian or Alaska Native | Asian | Black or African American | Native Hawaiian or Other Pacific Islander | White',
     N'American Indian', N'Alaska Native', N'Aleutian', N'TRUE', N'American Indian | Alaska Native | Aleutian | Apache',
     N'Chinese', N'Japanese', N'Korean', N'FALSE', N'Chinese | Japanese | Korean',
     N'African American', N'Black', N'Kenyan', N'FALSE', N'African American | Black | Kenyan',
     N'Native Hawaiian', N'Samoan', N'Tongan', N'FALSE', N'Native Hawaiian | Samoan | Tongan',
     N'European', N'Irish', N'Italian', N'FALSE', N'European | Irish | Italian',
     N'MRN20020010', N'Medical record number',
     N'ELECTRONIC', N'Yes', N'Not asked', N'Did not ask',
     N'Female', N'Variant additional gender info',
     N'1210310',
     N'American Indian or Alaska Native | Asian',
     N'CANADA', N'General Medical and Surgical Hospitals', N'English',
     10009282, N'Kent, Ariella', '2026-04-01T00:00:00',
     10009282, N'Kent, Ariella', '2026-04-01T00:00:00'),

-- v3 Deceased Patient (UID 20020020): exercises PATIENT_DECEASED_DATE +
-- PATIENT_DECEASED_INDICATOR='Yes' branch. Other columns minimal.
    (20020020, NULL, N'ACTIVE', N'PSN20020020GA01',
     NULL,
     N'Deceased', NULL, N'Patient', NULL,
     N'L', N'A', NULL,
     NULL, NULL, NULL, NULL, NULL,
     NULL, NULL, NULL, NULL, NULL,
     NULL,
     NULL, NULL, NULL, NULL,
     NULL, NULL,
     '1955-03-10T00:00:00', NULL, NULL, NULL,
     N'Male', N'Male', N'M',
     N'Yes', N'Y', '2025-12-15T00:00:00',             -- deceased branch
     NULL, NULL, NULL, NULL,
     N'Unknown', NULL,
     NULL, NULL, NULL, NULL, NULL,
     NULL, NULL, NULL, NULL, NULL,

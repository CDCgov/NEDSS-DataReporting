...
HEPATITIS_DATAMART / STD_HIV_DATAMART via the `#TMP_F_PAGE_CASE` isolation bug in
`013-sp_hepatitis_datamart` — are Step-9 SP outputs, not fixture writes, so they
...
**Doc corrections needed:** `coverage_tier_3.md` claims VAR/TB/COVID/STD datamart
SPs "all use the same TMP_F_PAGE_CASE pattern" — FALSE (F_PAGE_CASE refs per SP:
hepatitis 013 = 66; covid 310 = 0; var 250 = 0; tb 255 = 0). The isolation bug is
specific to the F_PAGE_CASE-dependent SPs (hepatitis). Also its claim that
HEPATITIS_CASE/HEPATITIS_DATAMART stay at 0 is stale (live: HEPATITIS_DATAMART=6,
HEPATITIS_CASE=1).
...
## Group B — whole file superseded by an existing ODSE-only sibling → quarantine + document
...
| `zz_enrich_phase2_investigations.sql` | high | Replace the 60-col nrt_investigation UPDATE (22 PHCs) with ODSE `public_health_case` column fills + `participation` InvestgrOfPHC + `nbs_case_answer` (for inv_state_case_id/legacy_case_id/city_county_case_nbr). 056 decodes the ~20 text columns — author raw codes only |
